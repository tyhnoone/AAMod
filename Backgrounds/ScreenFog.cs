...
namespace AAMod.Backgrounds
{
    public class ScreenFog
    {
        ...
        public void Update(Texture2D texture)
        {
            ...
            if (!backgroundFog && (BasePlayer.HasAccessory(player, AAMod.instance.ItemType("Lantern"), true, false) ||
            ...
            if (backgroundFog)
            {
                dayTimeOpacity = (Main.dayTime
                    ? BaseUtility.MultiLerp((float) Main.time / 52000f, 0.5f, 1f, 1f, 1f, 1f, 1f, 0.5f)
                    : 0.5f);
            ...
            {
                dayTimeOpacity = (Main.dayTime
                    ? BaseUtility.MultiLerp((float) Main.time / 52000f, 1f, 1f, 1f, 1f, 1f, 1f, 1f)
                    : 0.3f);
                ...
                for (int j = minY; j < maxY; j += texture.Height)
                {
                    if (player.position.Y < Main.worldSurface * 16.0)
                    {
                        Main.spriteBatch.Draw(texture,
                            new Rectangle(i + (dir ? -fogOffsetX : fogOffsetX), j, texture.Width, texture.Height), null,
