...
namespace AAMod.Backgrounds
{
    public class YamataSky : CustomSky
    {
        ...
        public override void Draw(SpriteBatch spriteBatch, float minDepth, float maxDepth)
        {
            if (maxDepth >= 3.40282347E+38f && minDepth < 3.40282347E+38f)
            {
                if (!Main.dayTime)
                {
                    ...
                    spriteBatch.Draw(SkyTex, SkyPos, null, new Color(102, 20, 80), 0f,
                        new Vector2(SkyTex.Width >> 1, SkyTex.Height >> 1), 1f, SpriteEffects.None, 1f);
                    ...
                    int num23 = (int) (Main.time / 32400.0 *
                                       (Main.screenWidth + Main.moonTexture[Main.moonType].Width * 2)) -
                    ...
                    Main.spriteBatch.Draw(PlanetTexture, new Vector2(num23, (num24 + Main.moonModY)),
                        new Rectangle?(new Rectangle(0, 0, PlanetTexture.Width, PlanetTexture.Width)), white2,
                        rotation2, new Vector2(PlanetTexture.Width / 2, PlanetTexture.Width / 2), num25,
            ...
            for (int j = num; j < num2; j++)
            {
                ...
                if (rectangle.Contains((int) vector.X, (int) vector.Y))
                {
                    ...
                    spriteBatch.Draw(BeamTexture, vector, null, Color.White * 0.2f * scale * Intensity, 0f,
                        Vector2.Zero, new Vector2(num3 / 70f, num3 / 45f), SpriteEffects.None, 0f);
                    int num4 = 0;
                    for (float num5 = 0f; num5 <= 1f; num5 += 0.03f)
                    {
                        float num6 = 1f - (num5 + Main.GlobalTime * 0.02f + (float) Math.Sin((double) ((float) j))) %
                                     1f;
                        spriteBatch.Draw(RockTextures[num4],
                            vector + new Vector2(
                                (float) Math.Sin((double) (num5 * 1582f)) * (num3 * 0.5f) + num3 * 0.5f, num6 * 2000f),
                            null, Color.White * num6 * scale * Intensity, num6 * 20f,
                            new Vector2((float) (RockTextures[num4].Width >> 1),
                                (float) (RockTextures[num4].Height >> 1)), 0.9f, SpriteEffects.None, 0f);
        ...
        public override void Activate(Vector2 position, params object[] args)
        {
            ...
            for (int i = 0; i < _pillars.Length; i++)
            {
                _pillars[i].Position.X = (float) i / (float) _pillars.Length * ((float) Main.maxTilesX * 16f + 20000f) +
                                         _random.NextFloat() * 40f - 20f - 20000f;
