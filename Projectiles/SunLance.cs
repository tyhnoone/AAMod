...
namespace AAMod.Projectiles
{
    public class SunLance : ModProjectile
    {
        ...
        {
            new Vector2(0, 0), new Vector2(50, -25), new Vector2(100, -50), new Vector2(100, 0), new Vector2(100, 50),
            new Vector2(50, 25), new Vector2(30, 0), new Vector2(150, 0), new Vector2(150, 0), new Vector2(30, 0)
        };
        ...
        {
            BaseMod.BaseDrawing.DrawProjectileSpear(sb, Main.projectileTexture[projectile.type], 0, projectile, null,
                0f, 0f);
            return false;
        }
        ...
        public static void AIArcStabSpear(Projectile p, ref float[] ai, bool overrideKill = false)
        {
            ...
            if (Main.myPlayer == p.owner && item != null && item.autoReuse && plr.itemAnimation == 1)
            {
                p.Kill();
                return;
            } //prevents a bug with autoReuse and spears
            ...
            AIArcStabSpear(p, ref ai, plr.Center + gfxOffset,
                BaseMod.BaseUtility.RotationTo(p.Center, p.Center + p.velocity), plr.direction, plr.itemAnimation,
                plr.itemAnimationMax, overrideKill, plr.frozen);
        }

        public static void AIArcStabSpear(Projectile p, ref float[] ai, Vector2 center, float itemRot,
            int ownerDirection, int itemAnimation, int itemAnimationMax, bool overrideKill = false, bool frozen = false)
        {
            if (p.timeLeft < 598) p.alpha -= 70;
            ...
            p.position += BaseMod.BaseUtility.RotateVector(default(Vector2),
                BaseMod.BaseUtility.MultiLerpVector(1f - (float) (itemAnimation / (float) itemAnimationMax), spearPos),
                itemRot);
