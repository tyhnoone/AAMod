    ...
    public class DaystormP : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] == 1f && projectile.ai[0] != 1f)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    int num5 = Dust.NewDust(value - Vector2.One * 8f, 16, 16, 135, projectile.velocity.X / 2f,
                        projectile.velocity.Y / 2f, 100, default(Color), 1f);
            ...
            if (flag && Main.myPlayer == projectile.owner)
            {
                ...
                if (flag2)
                {
                    ...
                    Vector2 value3 = Main.screenPosition + new Vector2((float) Main.mouseX, (float) Main.mouseY) -
                                     value2;
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        value2 = projectile.Center + new Vector2((float) Main.rand.Next(-num7, num7 + 1),
                        ...
                        spinningpoint = spinningpoint.RotatedBy(
                            Main.rand.NextDouble() * 0.19634954631328583 - 0.098174773156642914, default(Vector2));
                        if (float.IsNaN(spinningpoint.X) || float.IsNaN(spinningpoint.Y))
                        {
                            spinningpoint = -Vector2.UnitY;
                        }

                        Projectile.NewProjectile(value2.X, value2.Y, spinningpoint.X, spinningpoint.Y, num6,
            ...
            player.itemRotation = (float) Math.Atan2((double) (projectile.velocity.Y * (float) projectile.direction),
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
                    ...
                    .inventory[Main.player[projectile.owner].selectedItem].ranged)
            {
                ...
                color25 = color25.MultiplyRGBA(new Color(Vector4.Lerp(Vector4.One, new Vector4(0f, 0.12f, 0.16f, 0f),
                    1f - num217)));
                scale5 = num217;
            }

            Main.spriteBatch.Draw(texture2D14, vector27,
                new Rectangle?(new Rectangle(0, y7, texture2D14.Width, num215)), projectile.GetAlpha(color25),
                projectile.rotation, new Vector2(texture2D14.Width / 2f, num215 / 2f), projectile.scale, spriteEffects,
                0f);
            Main.spriteBatch.Draw(Glow, vector27, new Rectangle?(new Rectangle(0, y7, texture2D14.Width, num215)),
                AAColor.Glow * scale5, projectile.rotation, new Vector2(texture2D14.Width / 2f, num215 / 2f),
