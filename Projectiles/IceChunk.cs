    ...
    public class IceChunk : ModProjectile
    {
        ...
        public override void AI()
        {
            if (Main.rand.NextFloat() < 0.9210526f)
            {
                ...
                dust = Main.dust[
                    Dust.NewDust(position, 30, 30, mod.DustType<Dusts.SnowDust>(), 0f, 0f, 0, default(Color), 1)];
                dust.noGravity = true;
            }
        }
        ...
        public override void Kill(int timeleft)
        {
            for (int num468 = 0; num468 < 10; num468++)
            {
                int num469 = Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width,
                    projectile.height, mod.DustType<Dusts.SnowDust>(), -projectile.velocity.X * 0.2f,
                ...
                num469 = Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width,
                    projectile.height, mod.DustType<Dusts.IceDust>(), -projectile.velocity.X * 0.2f,
                    -projectile.velocity.Y * 0.2f, 0, default(Color), 1f);
                Main.dust[num469].velocity *= 2f;
            }
        }
