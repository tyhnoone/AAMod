...
namespace AAMod.Projectiles
{
    ...
    public static class ProjectileDraw
    {
        public static void HomingAI(ModProjectile modProj, NPC target, float velocity = 4f, float acceleration = 0.1f)
        {
            ...
            Vector2 aim = new Vector2(target.position.X + (target.width >> 1),
                target.position.Y + (target.height >> 1));
            Vector2 pos = new Vector2(projectile.position.X + (projectile.width >> 1),
                projectile.position.Y + (projectile.height >> 1));
            aim -= pos;
        ...
        public static void HomingAIPredictive(ModProjectile modProj, NPC target, float velocity = 4f,
        ...
        public static NPC FindCheapestNPC(Vector2 position, Vector2 velocity, float acceleration, float maxAngle,
            float maxDist = 2000f, bool ignoreLineOfSight = false, bool ignoreFriendlies = true,
            bool ignoreDontTakeDamage = false, bool ignoreChaseable = false)
        {
            ...
            for (int i = 0; i < 200; i++)
            {
                ...
                if (npc.active && (ignoreChaseable || npc.chaseable && npc.lifeMax > 5)
                               && (ignoreDontTakeDamage || !npc.dontTakeDamage) &&
                               (!ignoreFriendlies || !npc.friendly) && !npc.immortal)
                {
        ...
        public static NPC FindRandomNPC(Vector2 position, float maxDist, bool ignoreLineOfSight = true,
            bool ignoreFriendlies = true, bool ignoreDontTakeDamage = false, bool ignoreChaseable = false)
        {
            ...
            for (int i = 0; i < Main.maxNPCs; i++)
            {
                               ...
                               (!ignoreFriendlies || !npc.friendly) && !npc.immortal)
                {
                    ...
                    if (distCurrent < maxDist &&
                        (ignoreLineOfSight || Collision.CanHitLine(position, 0, 0, npcCenter, 0, 0)))
                    {
                        targets[next] = npc;
                        next++;
                    }
        ...
        public static void HomingAIVanilla(ModProjectile modProj, NPC target, float velocity = 4f,
            float weight = 0.0333f)
        {
            Projectile projectile = modProj.projectile;
            Vector2 pos = new Vector2(projectile.position.X + (projectile.width >> 1),
                projectile.position.Y + (projectile.height >> 1));
        ...
        public static NPC FindNearestNPC(Vector2 position, float maxDist, bool ignoreLineOfSight = true,
            bool ignoreFriendlies = true, bool ignoreDontTakeDamage = false, bool ignoreChaseable = false)
        {
            NPC nearest = null;
        ...
        public static void SpearAI(int index, float protractSpeed = 1.5f, float retractSpeed = 1.4f,
            ExtraAction action = null, ExtraAction initialize = null)
        {
            ...
            Vector2 vector = Main.player[projectile.owner]
                .RotatedRelativePoint(Main.player[projectile.owner].MountedCenter, true);
        ...
        public static void YoyoAI(int index, float seconds, float length, float acceleration = 14f,
            float rotationSpeed = 0.45f, ExtraAction action = null, ExtraAction initialize = null)
        {
            Projectile projectile = Main.projectile[index];
            bool flag = false;
            if (initialize != null && projectile.localAI[1] == 0f)
            ...
            if (projectile.ai[0] >= 0f)
            {
                ...
                if (projectile.owner == Main.myPlayer)
                {
                    if (!Main.player[projectile.owner].channel || Main.player[projectile.owner].stoned ||
                ...
                if (projectile.ai[0] >= 0f)
                {
                    if (flag3)
                    {
                        ...
                        else if (projectile.Center.Y < Main.player[projectile.owner].Center.Y &&
        ...
        public static void FlailAI(int index, float initialRange = 160f, float weaponOutRange = 300f,
            float retractRange = 100f, ExtraAction action = null, ExtraAction initialize = null)
        {
            Projectile projectile = Main.projectile[index];
            if (initialize != null && projectile.localAI[1] == 0f)
            {
        ...
        public static void BoomerangAI(int index, float retractTime = 30f, float speed = 9f,
            float speedAcceleration = 0.4f, ExtraAction action = null, ExtraAction initialize = null)
        {
            ...
            else
            {
                ...
                Vector2 vector = new Vector2(projectile.position.X + (projectile.width * 0.5f),
                    projectile.position.Y + (projectile.height * 0.5f));
                float num = Main.player[projectile.owner].position.X + (Main.player[projectile.owner].width / 2) -
                            vector.X;
                float num2 = Main.player[projectile.owner].position.Y + (Main.player[projectile.owner].height / 2) -
                             vector.Y;
                float num3 = (float) Math.Sqrt((num * num) + (num2 * num2));
                if (num3 > 3000f)
                    projectile.Kill();
                ...
                {
                    Rectangle rectangle = new Rectangle((int) projectile.position.X, (int) projectile.position.Y,
                        projectile.width, projectile.height);
                    Rectangle value = new Rectangle((int) Main.player[projectile.owner].position.X,
                        (int) Main.player[projectile.owner].position.Y, Main.player[projectile.owner].width,
                        Main.player[projectile.owner].height);
        ...
        public static void ThrowingKnifeAI(int index, int airTime = 20, ExtraAction action = null,
            ExtraAction initialize = null)
        {
            Projectile projectile = Main.projectile[index];
            if (initialize != null && projectile.localAI[1] == 0f)
            {
        ...
        public static void Explode(int index, int sizeX, int sizeY, ExtraAction visualAction = null)
        {
            ...
            if (visualAction == null)
            {
                ...
                for (int k = 0; k < 2; k++)
                {
                    ...
                    int num3 = Gore.NewGore(new Vector2(projectile.position.X, projectile.position.Y), default(Vector2),
                    ...
                    num3 = Gore.NewGore(new Vector2(projectile.position.X, projectile.position.Y), default(Vector2),
                        Main.rand.Next(61, 64), 1f);
                    Main.gore[num3].velocity *= scaleFactor;
        ...
        public static void DrawString(int index, Vector2 to = default(Vector2))
        {
            ...
            while (flag)
            {
                ...
                else
                {
                    ...
                    color = new Color((byte) (color.R * num10), (byte) (color.G * num10), (byte) (color.B * num10),
                        (byte) (color.A * num10));
                    Main.spriteBatch.Draw(Main.fishingLineTexture,
                        new Vector2(vector.X - Main.screenPosition.X + (Main.fishingLineTexture.Width * 0.5f),
                            vector.Y - Main.screenPosition.Y + (Main.fishingLineTexture.Height * 0.5f)) -
                        new Vector2(6f, 0f),
                        new Rectangle?(new Rectangle(0, 0, Main.fishingLineTexture.Width, (int) num5)), color, rotation,
        ...
        public static void DrawChain(int index, Vector2 to, string chainPath)
        {
            ...
            while (flag)
            {
                ...
                else
                {
                    ...
                    Main.spriteBatch.Draw(texture, vector - Main.screenPosition, sourceRectangle, color, rotation,
        ...
        public static void DrawAroundOrigin(int index, Color lightColor)
        {
            ...
            Vector2 origin = new Vector2(texture2D.Width * 0.5f,
                texture2D.Height / Main.projFrames[projectile.type] * 0.5f);
            SpriteEffects effects = (projectile.direction == -1) ? SpriteEffects.FlipHorizontally : SpriteEffects.None;
            Main.spriteBatch.Draw(texture2D, projectile.Center - Main.screenPosition,
                new Rectangle?(Utils.Frame(texture2D, 1, Main.projFrames[projectile.type], 0, projectile.frame)),
                lightColor, projectile.rotation, origin, projectile.scale, effects, 0f);
        }
        ...
        public static void DrawSpear(int index, Color lightColor)
        {
            ...
            Main.spriteBatch.Draw(Main.projectileTexture[projectile.type],
                ...
                new Rectangle?(new Rectangle(0, 0, Main.projectileTexture[projectile.type].Width,
                    Main.projectileTexture[projectile.type].Height)), projectile.GetAlpha(lightColor),
                projectile.rotation, zero, projectile.scale, effects, 0f);
        }
    }
}