    ...
    public class FreedomStar : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.ai[1] == 1f && projectile.ai[0] != 1f)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    int num5 = Dust.NewDust(value - Vector2.One * 8f, 16, 16, 74, projectile.velocity.X / 2f,
            ...
            if (flag && Main.myPlayer == projectile.owner)
            {
                if (player.channel && !player.noItems && !player.CCed)
                {
                    ...
                    Vector2 value2 = Main.screenPosition + new Vector2((float) Main.mouseX, (float) Main.mouseY) -
                                     vector3;
                    ...
                    vector3 = projectile.Center + new Vector2((float) Main.rand.Next(-num7, num7 + 1),
                                  (float) Main.rand.Next(-num7, num7 + 1));
                    Vector2 vector5 = Vector2.Normalize(projectile.velocity) * scaleFactor2;
                    vector5 = vector5.RotatedBy(Main.rand.NextDouble() * 0.19634954631328583 - 0.098174773156642914,
                        default(Vector2));
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                float num1 = 12f;
                Vector2 vector2 = new Vector2(player.position.X + (float) player.width * 0.5f,
                    player.position.Y + (float) player.height * 0.5f);
                ...
                switch (chargeLevel)
                {
                    case 0:
                        ...
                        Projectile.NewProjectile(vector2.X, vector2.Y, SpeedX, SpeedY,
                            mod.ProjectileType("FreedomShot"), projectile.damage, 1f, player.whoAmI);
                        break;
                    case 1:
                        Main.PlaySound(2, (int) projectile.position.X, (int) projectile.position.Y, 89);
                        Projectile.NewProjectile(vector2.X, vector2.Y, SpeedX, SpeedY,
                            mod.ProjectileType("FreedomShot"), projectile.damage * 2, 1f, player.whoAmI);
                        break;
                    case 2:
                    ...
                    case 3:
                        Main.PlaySound(2, (int) projectile.position.X, (int) projectile.position.Y, 88);
                        Projectile.NewProjectile(vector2.X, vector2.Y, SpeedX, SpeedY,
                            mod.ProjectileType("FreedomShotCharged"), projectile.damage * 6, 1f, player.whoAmI);
                        break;
                }
            }
        }
