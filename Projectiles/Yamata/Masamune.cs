...
namespace AAMod.Projectiles.Yamata
{
    class Masamune : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            {
                int num137 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
                    projectile.height, mod.DustType<Dusts.YamataDust>(), 0f, 0f, 0, default(Color), 1f);
            ...
            if (projectile.localAI[1] > 10f && Main.rand.Next(3) == 0)
            {
                ...
                {
                    Vector2 vector56 = Vector2.Normalize(projectile.velocity) *
                                       new Vector2((float) projectile.width, (float) projectile.height) / 2f;
                    vector56 = vector56.RotatedBy(
                                   (double) (num694 - (num693 / 2 - 1)) * 3.1415926535897931 /
                                   (double) ((float) num693), default(Vector2)) + projectile.Center;
                    Vector2 value24 =
                        ((float) (Main.rand.NextDouble() * 3.1415927410125732) - 1.57079637f).ToRotationVector2() *
                        (float) Main.rand.Next(3, 8);
                    int num695 = Dust.NewDust(vector56 + value24, 0, 0, mod.DustType<Dusts.YamataDust>(),
                        value24.X * 2f, value24.Y * 2f, 100, default(Color), 1.4f);
            ...
            if (projectile.ai[0] == 0f && projectile.localAI[0] == 0f)
            {
                for (int num698 = 0; num698 < 200; num698++)
                {
                    ...
                    if (nPC6.CanBeChasedBy(this, false) &&
                        (projectile.ai[0] == 0f || projectile.ai[0] == (float) (num698 + 1)))
                    {
                        Vector2 center4 = nPC6.Center;
                        float num699 = Vector2.Distance(center4, vector57);
                        if (num699 < num697 && Collision.CanHit(projectile.position, projectile.width,
                                projectile.height, nPC6.position, nPC6.width, nPC6.height))
                        {
                            num697 = num699;
                            vector57 = center4;
                            num696 = num698;
            ...
            if (projectile.ai[0] != 0f)
            {
                ...
                if (Main.npc[num700].active && !Main.npc[num700].dontTakeDamage &&
                    Main.npc[num700].immune[projectile.owner] == 0)
                {
                    ...
                    float num703 = Math.Abs(projectile.position.X + (float) (projectile.width / 2) - num701) +
                                   Math.Abs(projectile.position.Y + (float) (projectile.height / 2) - num702);
                    if (num703 < 1000f)
                    {
                        flag31 = true;
