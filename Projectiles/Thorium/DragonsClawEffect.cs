...
namespace AAMod.Projectiles.Thorium
{
    public class DragonsClawEffect : ModProjectile
    {
        ...
        {
            float newPosX = (float) (Math.Cos(rot) * (vecToRot.X - origin.X) - Math.Sin(rot) * (vecToRot.Y - origin.Y) +
                                     origin.X);
            float newPosY = (float) (Math.Sin(rot) * (vecToRot.X - origin.X) + Math.Cos(rot) * (vecToRot.Y - origin.Y) +
                                     origin.Y);
        ...
        public override void AI()
        {
            ...
            projectile.Center = player.Center + new Vector2(-8f, -8f) +
                                RotateVector(default(Vector2), rotVec, rot + (projectile.ai[0] * (6.28f / 2)));
            ...
            for (int m = 0; m < 5; m++)
            {
                ...
                int dustID = Dust.NewDust(projectile.position, projectile.width, projectile.height,
                    mod.DustType<Dusts.IncineriteDust>(), 0, 0, 0, default(Color), 1f);
