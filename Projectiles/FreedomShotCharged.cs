...
namespace AAMod.Projectiles
{
    public class FreedomShotCharged : ModProjectile
    {
        ...
        public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
        {
            ...
            {
                Projectile.NewProjectile(projectile.position, new Vector2(0, 0), mod.ProjectileType("FreedomBall"),
                    projectile.damage / 6, 0f, projectile.owner);
                firstHit = false;
            }
        }
        ...
        public override void Kill(int timeLeft)
        {
            ...
            int p = Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0f, 0f,
                mod.ProjectileType("DummyExplosionTerra"), projectile.damage, 0, Main.myPlayer);
            Main.projectile[p].magic = false;
            Main.projectile[p].ranged = true;
            for (int index1 = 0; index1 < 30; ++index1)
            {
                int index2 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
                    projectile.height, 74, 0.0f, 0.0f, 100, new Color(), 1f);
