...
namespace AAMod.Projectiles.GemShot
{
    class DiamondShot : ModProjectile
    {
        ...
        {
            Lighting.AddLight(projectile.Center, ((255 - projectile.alpha) * .8f) / 255f,
                ((255 - projectile.alpha) * .8f) / 255f, ((255 - projectile.alpha) * .8f) / 255f);
            ...
            for (int num339 = 0; num339 < 16; num339++)
            {
                ...
                dust1 = Main.dust[
                    Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width,
                        projectile.height, mod.DustType<Dusts.AbyssDust>(), 0, 0, 0, Color.Silver, 1f)];
