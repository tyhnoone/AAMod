...
namespace AAMod.Projectiles
{
    public class FerretNote : ModProjectile
    {
        ...
        public void HandleTargetingMovement(float rotScalar = 0.1f, float entVelScalar = 0.25f, float newVelSpeed = 11f,
            float maxSpeed = 11f)
        {
            Target();
            if (target != -1)
            {
                ...
                projectile.velocity += BaseMod.BaseUtility.RotateVector(default(Vector2), new Vector2(maxSpeed, 0f),
                                           BaseMod.BaseUtility.RotationTo(projectile.Center, ent.Center)) * rotScalar;
        ...
        public void Target()
        {
            ...
            if (target != -1 && !CanTarget(Main.npc[target]))
            {
                target = -1;
            }
            ...
            if (target == -1 && targetDelay == 0 && projectile.timeLeft % 20 == 0)
            {
                ...
                if (npcs.Length > 0)
                {
                    ...
                    foreach (int i in npcs)
                    {
                        ...
                        if (CanTarget(npc) && dist < prevDist)
                        {
                            target = npc.whoAmI;
                            prevDist = dist;
                        }
        ...
        public override void Kill(int timeLeft)
        {
            for (int m = 0; m < 10; m++)
            {
                int dustID = Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width, 1,
                    mod.DustType<Dusts.AkumaDustLight>(), -projectile.velocity.X * 0.2f,
                    -projectile.velocity.Y * 0.2f, 100, default(Color), 2f);
                Main.dust[dustID].noGravity = true;
                Main.dust[dustID].velocity *= 2f;
                dustID = Dust.NewDust(new Vector2(projectile.Center.X, projectile.Center.Y), projectile.width,
                    projectile.height, mod.DustType<Dusts.AkumaDustLight>(), -projectile.velocity.X * 0.2f,
