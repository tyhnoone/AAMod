...
namespace AAMod.Projectiles
{
    public class Spectrum : ModProjectile
    {
            ...
            if (flag10 && Main.myPlayer == projectile.owner)
            {
                ...
                if (flag12)
                {
                    if (projectile.ai[0] == 1f)
                    {
                        ...
                        for (int l = 0; l < 7; l++)
                        {
                            Projectile.NewProjectile(center3.X, center3.Y, vector12.X, vector12.Y,
                                mod.ProjectileType("SpectrumBeam"), num29, projectile.knockBack, projectile.owner,
                                (float) l, (float) projectile.whoAmI);
                        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            Vector2 vector27 = (projectile.position +
                    ...
                    .inventory[Main.player[projectile.owner].selectedItem].ranged)
            {
                ...
                color25 = color25.MultiplyRGBA(new Microsoft.Xna.Framework.Color(Vector4.Lerp(Vector4.One,
                    new Vector4(0.16f, 0.12f, 0f, 0f), 1f - num217)));
                scale5 = num217;
            }

            Main.spriteBatch.Draw(texture2D14, vector27,
                new Microsoft.Xna.Framework.Rectangle?(
                    new Microsoft.Xna.Framework.Rectangle(0, y7, texture2D14.Width, num215)),
                projectile.GetAlpha(color25), projectile.rotation,
                new Vector2((float) texture2D14.Width / 2f, (float) num215 / 2f), projectile.scale, spriteEffects, 0f);
            ...
            for (float num218 = 0f; num218 < 4f; num218 += 1f)
            {
                Main.spriteBatch.Draw(texture2D14,
                    vector27 + Vector2.UnitY.RotatedBy((double) (num218 * 6.28318548f / 4f), default(Vector2)) *
                    scaleFactor2,
                    new Microsoft.Xna.Framework.Rectangle?(
                        new Microsoft.Xna.Framework.Rectangle(0, y7, texture2D14.Width, num215)),
                    projectile.GetAlpha(color25).MultiplyRGBA(new Microsoft.Xna.Framework.Color(255, 255, 255, 0)) *
                    0.03f, projectile.rotation, new Vector2((float) texture2D14.Width / 2f, (float) num215 / 2f),
