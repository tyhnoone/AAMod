...
namespace AAMod.Projectiles
{
    public class SpectrumBeam : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (projectile.type != mod.ProjectileType("SpectrumBeam") ||
                !Main.projectile[(int) projectile.ai[1]].active ||
                Main.projectile[(int) projectile.ai[1]].type != mod.ProjectileType("Spectrum"))
            {
                projectile.Kill();
                return;
            }
            ...
            value37 = (Vector2.UnitY.RotatedBy((double) num814, default(Vector2)) * new Vector2(4f, y)).RotatedBy(
                (double) projectile2.velocity.ToRotation(), default(Vector2));
            projectile.position = projectile2.Center + value36 * 16f - projectile.Size / 2f +
                                  new Vector2(0f, -Main.projectile[(int) projectile.ai[1]].gfxOffY);
            ...
            if (Math.Abs(projectile.localAI[1] - num821) < 100f && projectile.scale > 0.15f)
            {
                ...
                color.A = 0;
                Vector2 vector80 = projectile.Center +
                                   projectile.velocity * (projectile.localAI[1] - 14.5f * projectile.scale);
                float x = Main.rgbToHsl(new Color(255, 250, 205)).X;
                for (int num843 = 0; num843 < 2; num843++)
                {
                    float num844 = projectile.velocity.ToRotation() +
                                   ((Main.rand.Next(2) == 1) ? -1f : 1f) * 1.57079637f;
                    float num845 = (float) Main.rand.NextDouble() * 0.8f + 1f;
                    Vector2 vector81 = new Vector2((float) Math.Cos((double) num844) * num845,
                        (float) Math.Sin((double) num844) * num845);
                    int num846 = Dust.NewDust(vector80, 0, 0, 261, vector81.X, vector81.Y, 0, new Color(255, 250, 205),
                        1f);
                    ...
                    Main.dust[num846].color =
                        Color.Lerp(color, Main.hslToRgb(0.54f, 1f, 0.902f), projectile.scale / 1.4f);
                }

                if (Main.rand.Next(5) == 0)
                {
                    Vector2 value42 = projectile.velocity.RotatedBy(1.5707963705062866, default(Vector2)) *
                                      ((float) Main.rand.NextDouble() - 0.5f) * (float) projectile.width;
                    int num847 = Dust.NewDust(vector80 + value42 - Vector2.One * 4f, 8, 8, 261, 0f, 0f, 100,
                        new Color(255, 250, 205), 1f);
                ...
                Vector2 size = new Vector2(projectile.velocity.Length() * projectile.localAI[1],
                ...
                if (Main.netMode != 2)
                {
                    ((WaterShaderData) Filters.Scene["WaterDistortion"].GetShader()).QueueRipple(
                        projectile.position +
                        new Vector2(size.X * 0.5f, 0f).RotatedBy((double) num848, default(Vector2)),
                        new Color(0.5f, 0.1f * (float) Math.Sign(value43) + 0.5f, 0f, 1f) * Math.Abs(value43), size,
                        RippleShape.Square, num848);
                }

                Utils.PlotTileLine(projectile.Center, projectile.Center + projectile.velocity * projectile.localAI[1],
                    (float) projectile.width * projectile.scale, new Utils.PerLinePoint(DelegateMethods.CastLight));
                return;
            }
        }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            ...
            projPos = new Vector2((float) projectile.width, (float) projectile.height) / 2f +
                      Vector2.UnitY * projectile.gfxOffY - Main.screenPosition;
            Utils.DrawLaser(Main.spriteBatch, tex, value26 - Main.screenPosition,
                value26 + projectile.velocity * num228 - Main.screenPosition, vector29,
            ...
            Utils.DrawLaser(Main.spriteBatch, tex, value26 - Main.screenPosition,
                value26 + projectile.velocity * num228 - Main.screenPosition, vector29 / 2f,
                new Utils.LaserLineFraming(DelegateMethods.RainbowLaserDraw));
            return false;
        }
        ...
        public override void CutTiles()
        {
            ...
            Utils.PlotTileLine(projectile.Center, projectile.Center + unit * projectile.localAI[1],
                (float) projectile.width * projectile.scale, new Utils.PerLinePoint(DelegateMethods.CutTiles));
        }
        ...
        public override bool? Colliding(Rectangle projHitbox, Rectangle targetHitbox)
        {
            ...
            if (Collision.CheckAABBvLineCollision(targetHitbox.TopLeft(), targetHitbox.Size(), projectile.Center,
                projectile.Center + projectile.velocity * projectile.localAI[1], 22f * projectile.scale, ref num6))
            {
                return true;
            }
