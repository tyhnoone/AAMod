...
namespace AAMod.Projectiles
{
    public class SoulSiphonHeal : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (num491 < 50f &&
                projectile.position.X < Main.player[player].position.X + (float) Main.player[player].width &&
                projectile.position.X + (float) projectile.width > Main.player[player].position.X &&
                projectile.position.Y < Main.player[player].position.Y + (float) Main.player[player].height &&
                projectile.position.Y + (float) projectile.height > Main.player[player].position.Y)
            {
            ...
            for (int num493 = 0; num493 < 3; num493++)
            {
                ...
                int num496 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
                    projectile.height, DustID.Shadowflame, 0f, 0f, 100, default(Color), 1.1f);
            ...
            for (int num497 = 0; num497 < 5; num497++)
            {
                ...
                int num500 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
                    projectile.height, DustID.Shadowflame, 0f, 0f, 100, default(Color), 1.3f);
