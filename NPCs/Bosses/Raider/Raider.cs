...
namespace AAMod.NPCs.Bosses.Raider
{
    ...
    public class Raider : ModNPC
    {
        ...
        public override void NPCLoot()
        {
            ...
            if (Main.rand.Next(10) == 0)
            {
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height,
                    mod.ItemType("RaiderTrophy"));
            }

            if (Main.expertMode)
            {
                npc.DropBossBags();
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height, ItemID.SoulofFright,
            ...
            else
            {
                ...
                {
                    Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height,
                        mod.ItemType("RaidEgg"));
                }

                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height,
                    mod.ItemType("FulguriteBar"), Main.rand.Next(30, 64));
            }
        }
        ...
        public const float
            AISTATE_FLYBACKTOPLAYER =
                1f; //uses this to fly back to the player after laying eggs, no idea why it's the second state lol
        ...
        public override void FindFrame(int frameHeight)
        {
            ...
            if (npc.frameCounter > 8)
            {
                ...
                npc.frame.Y += 192;
                bool isCharging =
                    (npc.ai[0] >= AISTATE_CHARGEATPLAYER &&
                     npc.ai[0] < AISTATE_SPAWNEGGS); //all ai states between charges
                if (isCharging && npc.frame.Y >= 192 * 8)
                {
                    npc.frame.Y = 192 * 4;
                }
                else if (!isCharging && npc.frame.Y >= 192 * 4)
                {
                    npc.frame.Y = 192 * 0;
                }
            }
        ...
        {
            if (Main.netMode != 1 && npc.ai[0] == AISTATE_FLYABOVEPLAYER
            ) //only fire bombs when (attempting to) fly above the player
            {
                projectileTimer++;
                if (projectileTimer >= projectileInterval && projectileTimer % 10 == 0)
                {
                    ...
                    firePos = BaseMod.BaseUtility.RotateVector(npc.Center, firePos,
                        npc.rotation); //+ (npc.direction == -1 ? (float)Math.PI : 0f)));
                    int projID = 0;
                    int npcID = 0;
                    if (ProjectileChoice == 1 || ProjectileChoice == 3 || ProjectileChoice == 4 ||
                        ProjectileChoice == 6 || ProjectileChoice == 8 || ProjectileChoice == 10)
                    {
                        int RocketNumber = 5;
                    ...
                    {
                        projID = Projectile.NewProjectile(firePos, dir, mod.ProjectileType("Raidsphere"),
                            npc.damage / 2, 1, 255);
            ...
            else if (npc.ai[0] == AISTATE_FLYABOVEPLAYER)
            {
                npc.TargetClosest(true);
                    ...
                    while (npc.ai[0] == 0f)
                    {
                        ...
                        else if (num1307 == 1)
                        {
                            npc.ai[0] = 3f;
                                 ...
                                 NPC.CountNPCS(mod.NPCType("RaidEgg")) + NPC.CountNPCS(mod.NPCType("Raidmini")) <
                                 numberOfMinions)
                        {
            ...
            else if (npc.ai[0] == AISTATE_FLYBACKTOPLAYER)
            {
                npc.collideX = false;
                npc.collideY = false;
                npc.noTileCollide = true;
            ...
            else if (npc.ai[0] == AISTATE_FLYTOPLAYER)
            {
            ...
            else if (npc.ai[0] == AISTATE_CHARGEATPLAYER)
            {
                npc.knockBackResist = 0f;
                npc.noTileCollide = true;
                if (npc.velocity.X < 0f)
                ...
                if (Math.Abs(npc.Center.X - Main.player[npc.target].Center.X) > 350f &&
                    Math.Abs(npc.Center.Y - Main.player[npc.target].Center.Y) < 20f)
                {
                    npc.ai[0] = 3.1f;
                    npc.ai[1] = 0f;
                }
            ...
            else if (npc.ai[0] == 3.1f) //sub charge
            {
                npc.knockBackResist = 0f;
                npc.noTileCollide = true;
            ...
            else if (npc.ai[0] == 3.2f) //sub charge
            {
                ...
                if ((npc.ai[1] > 0f && npc.Center.X > Main.player[npc.target].Center.X + 260f) ||
                    (npc.ai[1] < 0f && npc.Center.X < Main.player[npc.target].Center.X - 260f))
                {
            ...
            else if (npc.ai[0] == AISTATE_SPAWNEGGS)
            {
                npc.ai[0] = 0f;
                npc.TargetClosest(true);
                if (Main.netMode != 1)
                {
                    ...
                    for (int num1312 = 0; num1312 < 1000; num1312++)
                    {
                        ...
                        if (!WorldGen.SolidTile(num1313, num1314))
                        {
                            ...
                            if ((new Vector2((float) ((num1313 * 16) + 8), (float) ((num1314 * 16) + 8)) -
                                 Main.player[npc.target].Center).Length() < 600f)
                            {
                                npc.ai[0] = 4.1f;
            ...
            else if (npc.ai[0] == 4.1f) //sub spawning eggs
            {
            ...
            else if (npc.ai[0] == 4.2f) //sub spawning eggs
            {
                npc.knockBackResist = 0f;
                npc.noTileCollide = true;
                ...
                if (Main.netMode != 1 && vector208.Length() < 4f)
                {
                    ...
                    {
                        NPC.NewNPC((num1321 * 16) + 8, num1322 * 16, mod.NPCType("RaidEgg"), npc.whoAmI, 0f, 0f, 0f, 0f,
                            255);
                    }
                    else if (npc.ai[3] == (float) (num1325 * 2))
                    {
                        ...
                        npc.ai[2] = 0f;
                        npc.ai[3] = 0f;
                        if (NPC.CountNPCS(mod.NPCType("RaidEgg")) + NPC.CountNPCS(mod.NPCType("Raidmini")) <
                            numberOfMinions && Main.rand.Next(3) != 0)
                        {
                            npc.ai[0] = 4f;
                        }
