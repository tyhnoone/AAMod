...
namespace AAMod.NPCs.Bosses.Raider
{
    public class RaidShock : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Vector2.Distance(projectile.Center,
                    new Vector2(projectile.ai[0], projectile.ai[1]) * 16f + Vector2.One * 8f) <= 16f)
            {
                projectile.Kill();
                return;
            }
            ...
            if (projectile.alpha == 0)
            {
                ...
                if (projectile.localAI[0] == 3f)
                {
                    ...
                    for (int num53 = 0; num53 < 8; num53++)
                    {
                        ...
                        vector7 += -Vector2.UnitY.RotatedBy((double) ((float) num53 * 3.14159274f / 4f),
                                       default(Vector2)) * new Vector2(2f, 4f);
                        vector7 = vector7.RotatedBy((double) (projectile.rotation - 1.57079637f), default(Vector2));
                        int num54 = Dust.NewDust(projectile.Center, 0, 0, mod.DustType<Dusts.FulguriteDust>(), 0f, 0f,
                            0, default(Color), 1f);
        ...
        public override void Kill(int timeLeft)
        {
            ...
            {
                int num292 = Dust.NewDust(projectile.position, projectile.width, projectile.height,
                    mod.DustType<Dusts.FulguriteDust>(), 0f, 0f, 100, default(Color), 2.1f);
            ...
            {
                Rectangle value19 =
                    new Rectangle((int) projectile.Center.X - 40, (int) projectile.Center.Y - 40, 80, 80);
                for (int num293 = 0; num293 < 1000; num293++)
                {
                    if (num293 != projectile.whoAmI && Main.projectile[num293].active &&
                        Main.projectile[num293].owner == projectile.owner && Main.projectile[num293].type == 443 &&
                        Main.projectile[num293].getRect().Intersects(value19))
                    {
                ...
                Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, 0f, 0f, 443, projectile.damage, 0f,
