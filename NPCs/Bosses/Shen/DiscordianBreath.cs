    ...
    public class DiscordianBreath : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            Vector2 center = nPC.Center +
                             new Vector2((110f + num) * (float) nPC.spriteDirection, 30f).RotatedBy(
                                 (double) nPC.rotation, default(Vector2));
            ...
            Utils.PlotTileLine(projectile.Center + (projectile.rotation.ToRotationVector2() * 400f * num3),
                projectile.Center + (projectile.rotation.ToRotationVector2() * 400f * num2), 16f,
                new Utils.PerLinePoint(DelegateMethods.CastLight));
            Utils.PlotTileLine(
                projectile.Center +
                (projectile.rotation.ToRotationVector2().RotatedBy(0.19634954631328583, default(Vector2)) * 400f *
                 num3),
                projectile.Center +
                (projectile.rotation.ToRotationVector2().RotatedBy(0.19634954631328583, default(Vector2)) * 400f *
            ...
            if (num3 == 0f && num2 > 0.1f)
            {
                for (int i = 0; i < 3; i++)
                {
                    ...
                    Dust dust = Dust.NewDustDirect(projectile.position, projectile.width, projectile.height, NUM1, 0f,
                        0f, 0, default(Color), 1f);
                    dust.fadeIn = 1.5f;
                    dust.velocity =
                        projectile.rotation.ToRotationVector2()
                            .RotatedBy((double) (Main.rand.NextFloatDirection() * 0.2617994f), default(Vector2)) *
                        (0.5f + (Main.rand.NextFloat() * 2.5f)) * 15f;
            ...
            for (int j = 0; j < 1; j++)
            {
                ...
                dust2.position =
                    Vector2.Lerp(projectile.Center + (projectile.rotation.ToRotationVector2() * 400f * num3),
                        projectile.Center + (projectile.rotation.ToRotationVector2() * 400f * num2), num4);
                dust2.position +=
                    projectile.rotation.ToRotationVector2().RotatedBy(1.5707963705062866, default(Vector2)) *
                    (20f + (100f * (num4 - 0.5f)));
            }
