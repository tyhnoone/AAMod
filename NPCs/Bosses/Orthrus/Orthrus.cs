...
namespace AAMod.NPCs.Bosses.Orthrus
{
    ...
    public class Orthrus : YamataBoss
    {
        ...
        public override void NPCLoot()
        {
            ...
            {
                npc.DropBossBags();
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height, ItemID.SoulofMight,
        ...
        public override void AI()
        {
            ...
            if (HeadsSpawned &&
                (!NPC.AnyNPCs(mod.NPCType<OrthrusHead1>()) || !NPC.AnyNPCs(mod.NPCType<OrthrusHead2>())))
            {
                npc.NPCLoot();
                npc.active = false;
            }
            ...
            else
            {
                if (internalAI[1] == AISTATE_TURRET)
                {
                    ...
                    if (!HeadsSpawned)
                    {
                        if (Main.netMode != 1)
                        {
                            ...
                            latestNPC = NPC.NewNPC((int) npc.Center.X + 34, (int) npc.Center.Y - 23,
                                mod.NPCType("OrthrusHead1"), 0, npc.whoAmI);
                            Main.npc[latestNPC].realLife = npc.whoAmI;
                            Main.npc[latestNPC].ai[0] = npc.whoAmI;
                            Head1 = Main.npc[latestNPC];
                            latestNPC = NPC.NewNPC((int) npc.Center.X - 34, (int) npc.Center.Y - 23,
                                mod.NPCType("OrthrusHead2"), 0, npc.whoAmI);
                            Main.npc[latestNPC].realLife = npc.whoAmI;
                            Main.npc[latestNPC].ai[0] = npc.whoAmI;
                ...
                else if (internalAI[1] == AISTATE_FLY)
                {
                    ...
                    ) //make it less then what makes it rise so it doesn't keep locking between them
                    {
        ...
        public override bool PreDraw(SpriteBatch sb, Color dColor)
        {
            DrawHead(sb, "NPCs/Bosses/Orthrus/OrthrusHead1", "NPCs/Bosses/Orthrus/OrthrusHead1_Glow", Head1, dColor,
                false);
            DrawHead(sb, "NPCs/Bosses/Orthrus/OrthrusHead2", "NPCs/Bosses/Orthrus/OrthrusHead2_Glow", Head2, dColor,
                true);
