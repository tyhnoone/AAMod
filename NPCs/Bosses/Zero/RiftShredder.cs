    ...
    public class RiftShredder : ModNPC
    {
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (flag && Main.netMode != 1)
            {
                int ind = NPC.NewNPC((int) (npc.position.X + (double) (npc.width / 2)),
                    (int) npc.position.Y + (npc.height / 2), mod.NPCType("TeslaHand"), npc.whoAmI, 1.5f, npc.ai[1], 0f,
                    0f, byte.MaxValue);
                Main.npc[ind].life = 1;
        ...
        public override void AI()
        {
            ...
            else if (npc.ai[2] == 0.0 || npc.ai[2] == 3.0)
            {
                ...
                Vector2 vector2_3 = new Vector2(npc.position.X + (npc.width * 0.5f),
                    npc.position.Y + (npc.height * 0.5f));
            ...
            {
                if (npc.ai[2] != 5.0 ||
                    (npc.velocity.X <= 0.0 || npc.position.X + (double) (npc.width / 2) <=
                     Main.player[npc.target].position.X + (double) (Main.player[npc.target].width / 2)) &&
                    (npc.velocity.X >= 0.0 || npc.position.X + (double) (npc.width / 2) >=
                     Main.player[npc.target].position.X + (double) (Main.player[npc.target].width / 2)))
                    return;
                npc.ai[2] = 0.0f;
