...
namespace AAMod.NPCs.Bosses.Zero.Protocol
{
    ...
    public class ZeroAwakened : ModNPC
    {
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            Projectile.NewProjectile((new Vector2(npc.Center.X, npc.Center.Y)), (new Vector2(0f, 0f)),
                mod.ProjectileType("ZeroDeath1"), 0, 0);
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            {
                Panic = true;
                Main.NewText("WARNING. DRASTIC DAMAGE DETECTED, FAILURE IMMINENT. ENGAGE T0TAL 0FFENCE PR0T0C0L",
            ...
            if (npc.life <= npc.lifeMax / 3 && Panic == false && AAWorld.downedZero)
            {
                Panic = true;
                Main.NewText(
                    "WARNING. DRASTIC DAMAGE DETECTED, FAILURE IMMINENT AGAIN. ENGAGE T0TAL 0FFENCE PR0T0C0L 0MEGA",
                    Color.Red.R, Color.Red.G, Color.Red.B);
            }
        ...
        public override bool PreDraw(SpriteBatch spritebatch, Color dColor)
        {
            ...
            for (int k = 0; k < npc.oldPos.Length; k++)
            {
                ...
                spritebatch.Draw(ZeroTrail, npc.position, null, color, npc.rotation, drawOrigin, npc.scale,
        ...
        public override void AI()
        {
            ...
            float num367 = npc.position.X + (npc.width / 2) - Main.player[npc.target].position.X -
                           (Main.player[npc.target].width / 2);
            float num368 = npc.position.Y + npc.height - 59f - Main.player[npc.target].position.Y -
                           (Main.player[npc.target].height / 2);
            ...
            {
                int num371 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y + (npc.height * 0.25f)), npc.width,
                    (int) (npc.height * 0.5f), 5, npc.velocity.X, 2f, 0, default(Color), 1f);
            ...
            if (Main.netMode != 1 && !dead2 && npc.timeLeft < 10)
            {
                ...
                {
                    if (num372 != npc.whoAmI && Main.npc[num372].active &&
                        (Main.npc[num372].type == 125 || Main.npc[num372].type == 126) &&
                        Main.npc[num372].timeLeft - 1 > npc.timeLeft)
                    {
            ...
            else if (Math.Abs(npc.position.X - Main.player[npc.target].position.X) > 6000f ||
            ...
            else if (npc.ai[0] == 0f)
            {
                if (npc.ai[1] == 0f)
                {
                    ...
                    float num374 = 0.115f;
                    int num375 = 1;
                    Vector2 vector36 = new Vector2(npc.position.X + (npc.width * 0.5f),
                        npc.position.Y + (npc.height * 0.5f));
                    if (npc.position.X + (npc.width / 2) <
                        Main.player[npc.target].position.X + Main.player[npc.target].width)
                    {
                        num375 = -1;
                    }

                    float num376 = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) +
                                   (num375 * 300) - vector36.X;
                    float num377 = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) - 300f -
                                   vector36.Y;
                    ...
                    else if (npc.position.Y + npc.height < Main.player[npc.target].position.Y && num379 < 400f)
                    {
                        ...
                        {
                            npc.ai[3] = 0f;
                            vector36 = new Vector2(npc.position.X + (npc.width * 0.5f),
                                npc.position.Y + (npc.height * 0.5f));
                            num376 = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) -
                                     vector36.X;
                            num377 = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) -
                                     vector36.Y;
                            if (Main.netMode != 1)
                            {
                                float num380 = 9f;
                                ...
                                Projectile.NewProjectile(vector36.X, vector36.Y, num376, num377, num382, num381, 0f,
                                    Main.myPlayer, 0f, 0f);
                            }
                        }
                    }
                ...
                else if (npc.ai[1] == 1f)
                {
                    ...
                    Vector2 vector37 = new Vector2(npc.position.X + (npc.width * 0.5f),
                        npc.position.Y + (npc.height * 0.5f));
                    float num384 = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) -
                                   vector37.X;
                    float num385 = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) -
                                   vector37.Y;
            ...
            else if (npc.ai[0] == 1f || npc.ai[0] == 2f)
            {
                ...
                if (npc.ai[1] == 100f)
                {
                    ...
                    else
                    {
                        ...
                        {
                            Dust.NewDust(npc.position, npc.width, npc.height, 5, Main.rand.Next(-30, 31) * 0.2f,
                                Main.rand.Next(-30, 31) * 0.2f, 0, default(Color), 1f);
                        }
            ...
            else
            {
                ...
                if (npc.ai[1] == 0f)
                {
                    ...
                    Vector2 vector38 = new Vector2(npc.position.X + (npc.width * 0.5f),
                        npc.position.Y + (npc.height * 0.5f));
                    float num391 = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) -
                                   vector38.X;
                    float num392 = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) - 300f -
                                   vector38.Y;
                    ...
                    if (Main.netMode != 1)
                    {
                        ...
                        if (npc.localAI[1] > 180f && Collision.CanHit(npc.position, npc.width, npc.height,
                                ...
                                Main.player[npc.target].height))
                        {
                            ...
                            Vector2 PlayerDistance = new Vector2(npc.position.X + npc.width * 0.5f,
                                npc.position.Y + npc.height * 0.5f);
                            float PlayerPosX = Main.player[npc.target].position.X +
                                               (Main.player[npc.target].width / 2) + (num429 * 180) - PlayerDistance.X;
                            float PlayerPosY = Main.player[npc.target].position.Y +
                                               (Main.player[npc.target].height / 2) - PlayerDistance.Y;
                            float PlayerPos = (float) Math.Sqrt((PlayerPosX * PlayerPosX) + (PlayerPosY * PlayerPosY));
                            float num433 = 6f;
                            PlayerDistance = new Vector2(npc.position.X + npc.width * 0.5f,
                                npc.position.Y + npc.height * 0.5f);
                            PlayerPosX = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) -
                                         PlayerDistance.X;
                            PlayerPosY = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) -
                                         PlayerDistance.Y;
                            ...
                            for (int i = 0; i < 5; ++i)
                            {
                                if (Main.netMode != 1)
                                {
                                    Projectile.NewProjectile(PlayerDistance.X, PlayerDistance.Y, PlayerPosX, PlayerPosY,
                                        Proj, (int) (damage * 1.5f), 0f, Main.myPlayer);
                                }
                            }
                ...
                else
                {
                    ...
                    Vector2 vector39 = new Vector2(npc.position.X + (npc.width * 0.5f),
                        npc.position.Y + (npc.height * 0.5f));
                    float num400 = Main.player[npc.target].position.X + (Main.player[npc.target].width / 2) +
                                   (num397 * 340) - vector39.X;
                    float num401 = Main.player[npc.target].position.Y + (Main.player[npc.target].height / 2) -
                                   vector39.Y;
                    ...
                    if (Main.netMode != 1)
                    {
                        ...
                        if (npc.localAI[1] > 60f && Collision.CanHit(npc.position, npc.width, npc.height,
                                Main.player[npc.target].position, Main.player[npc.target].width,
                                Main.player[npc.target].height))
                        {
                            npc.localAI[1] = 0f;
                            ...
                            Projectile.NewProjectile(vector39.X, vector39.Y, num400, num401, num405, num404, 0f,
                                Main.myPlayer, 0f, 0f);
                        }
                    }
        ...
        public void Attack(int Attack)
        {
            ...
            else if (Attack == 1)
            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float) (Math.Sin(offsetAngle) * 4f),
                        (float) (Math.Cos(offsetAngle) * 2f), mod.ProjectileType("GlitchRocket"), npc.damage / 2, 0,
                        Main.myPlayer, 0f, 0f);
                }
            }
            else if (Attack == 2)
            {
                ...
                for (int i = 0; i < 5; i++)
                {
                    offsetAngle = (startAngle + deltaAngle * (i + i * i) / 2f) + 32f * i;
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float) (Math.Sin(offsetAngle) * 4f),
                        (float) (Math.Cos(offsetAngle) * 2f), mod.ProjectileType("Error"), npc.damage / 2, 0,
                        Main.myPlayer, 0f, 0f);
                }
            }
            else if (Attack == 3)
            {
                ...
                {
                    offsetAngle = (startAngle + deltaAngle * (i + i * i) / 2f) + 32f * i;
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, (float) (Math.Sin(offsetAngle) * 2),
                        (float) (Math.Cos(offsetAngle)), mod.ProjectileType("StaticSphere"), npc.damage / 2, 0,
                        Main.myPlayer, 0f, 0f);
                }
            }
        }
