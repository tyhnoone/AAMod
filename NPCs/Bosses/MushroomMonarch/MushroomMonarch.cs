    ...
    public class MushroomMonarch : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1)
            {
                if (!Main.dayTime)
                {
                    Projectile.NewProjectile(npc.Center, new Vector2(0f, 0f), mod.ProjectileType("MonarchRUNAWAY"), 0,
                        0);
                    npc.active = false;
                    return;
                }
                ...
                else if (!Collision.CanHit(npc.position, npc.width, npc.height, Main.player[npc.target].position,
            ...
            {
                AAAI.InfernoFighterAI(npc, ref npc.ai, true, false, 0, 0.07f, 3f, 3, 4, 60, true, 10, 60, true, null,
                    false);
            }
            else if (internalAI[1] == AISTATE_JUMP) //jumper
            {
        ...
        public override void BossLoot(ref string name, ref int potionType)
        {
            ...
            {
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height, mod.ItemType("Mushium"),
