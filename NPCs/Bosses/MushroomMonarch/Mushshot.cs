        ...
        public override void AI()
        {
            for (int num189 = 0; num189 < 1; num189++)
            {
                int num190 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y), projectile.width,
                    projectile.height, mod.DustType<Dusts.ShroomDust>(), 0f, 0f, 0, default(Color), 1f);
        ...
        public override void Kill(int timeLeft)
        {
            for (int dust = 0; dust <= 3; dust++)
            {
                Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height,
                    mod.DustType<Dusts.ShroomDust>(), projectile.oldVelocity.X * 0.5f, projectile.oldVelocity.Y * 0.5f);
            }
        }
    }
}