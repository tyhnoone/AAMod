    ...
    public class AkumaA : ModNPC
    {
        ...
        public override bool PreAI()
        {
            ...
            if (npc.life <= npc.lifeMax / 3 && Panic == false && npc.type == mod.NPCType<AkumaA>())
            {
                ...
                Main.NewText(
                    AAWorld.downedAkuma
                        ? "Still got it, do you? Ya got fire in your spirit! I like that about you, kid!"
                        : "What?! How have you lasted this long?! Why you little... I refuse to be bested by a terrarian again! Have at it!",
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    {
                        if (segment == 0 || segment == 2 || segment == 3 || segment == 5 || segment == 6 ||
                            segment == 8)
                        {
                            latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("AkumaABody"),
                        ...
                        {
                            latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("AkumaAArms"),
                                npc.whoAmI, 0, latestNPC);
                            Main.npc[latestNPC].realLife = npc.whoAmI;
                            Main.npc[latestNPC].ai[3] = npc.whoAmI;
                            segment += 1;
                    ...
                    latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("AkumaABody1"),
                        npc.whoAmI, 0, latestNPC);
                    Main.npc[latestNPC].realLife = npc.whoAmI;
                    Main.npc[latestNPC].ai[3] = npc.whoAmI;

                    latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("AkumaATail"),
                        npc.whoAmI, 0, latestNPC);
                    Main.npc[latestNPC].realLife = npc.whoAmI;
                    Main.npc[latestNPC].ai[3] = npc.whoAmI;
            ...
            if (!Main.dayTime)
            {
                Main.NewText("Nighttime won't save you from me this time, kid! The day is born anew!",
                    Color.DeepSkyBlue);
                Main.dayTime = true;
                Main.time = 0;
            }
        ...
        public override void NPCLoot()
        {
            if (Main.expertMode)
            {
                if (!AAWorld.downedAkuma)
                {
                    Item.NewItem((int) npc.Center.X, (int) npc.Center.Y, npc.width, npc.height,
                        mod.ItemType("DraconianRune"));
                }

                Main.NewText(
                    AAWorld.downedAkuma
                        ? "Heh, not too shabby this time kid. I'm impressed. Here. Take your prize."
                        : "GRAH..! HOW!? HOW COULD I LOSE TO A MERE MORTAL TERRARIAN?! Hmpf...fine kid, you win, fair and square. Heere's your reward.",
                    Color.DeepSkyBlue.R, Color.DeepSkyBlue.G, Color.DeepSkyBlue.B);
            ...
            Main.NewText("Nice. You cheated. Now come fight me in expert mode like a real man.", Color.DeepSkyBlue.R,
        ...
        {
            Player player = Main.player[npc.target];
            if (internalAI[1] == 1 || internalAI[1] == 7 || internalAI[1] == 15 || internalAI[1] == 18 ||
                internalAI[1] == 21)
            {
                if (!QuoteSaid)
                {
                    Main.NewText(
                        (!Quote1) ? "Sky's fallin' again! On your toes!" : "Down comes the flames of fury again!",
                        Color.DeepSkyBlue);
                    QuoteSaid = true;
                    Quote1 = true;
                }
            ...
            if ((internalAI[1] == 2 || internalAI[1] == 6 || internalAI[1] == 12 || internalAI[1] == 16 ||
                 internalAI[1] == 24))
            {
                if (!QuoteSaid)
                {
                    Main.NewText(
                        (!Quote1)
                            ? "You underestimate the artillery of a dragon, kid!"
                            : "Flames don't give in till the end, kid!", Color.DeepSkyBlue);
                    QuoteSaid = true;
                    Quote1 = true;
                }
                ...
                if (npc.ai[2] == 350)
                {
                    ...
                    for (int i = 0; i < Fireballs; i++)
                    {
                        ...
                        Projectile.NewProjectile(npc.Center.X, npc.Center.Y,
                            ...
                            baseSpeed * (float) Math.Cos(offsetAngle) * 2, mod.ProjectileType<AkumaABomb>(),
                            npc.damage / (Main.expertMode ? 2 : 4), 3, Main.myPlayer);
                    }
                }
            }

            if (internalAI[1] == 3 || internalAI[1] == 8 || internalAI[1] == 11 || internalAI[1] == 17 ||
                internalAI[1] == 23)
            {
            ...
            if (internalAI[1] == 4 || internalAI[1] == 10 || internalAI[1] == 13 || internalAI[1] == 20 ||
                internalAI[1] == 25)
            {
                if (npc.ai[2] == 350)
                {
            ...
            if (internalAI[1] == 5 || internalAI[1] == 9 || internalAI[1] == 14 || internalAI[1] == 19 ||
                internalAI[1] == 22)
            {
                if (!QuoteSaid)
                {
                    Main.NewText(
                        (!Quote1) ? "Hey Kid? Like Fireworks? No? Too Bad!" : "Here comes the grand finale, kid!",
                        Color.DeepSkyBlue);
                    QuoteSaid = true;
                    Quote1 = true;
                }
                ...
                if (npc.ai[2] == 350)
                {
                    Projectile.NewProjectile(npc.Center.X, npc.Center.Y, npc.velocity.X * 2, npc.velocity.Y,
                        mod.ProjectileType<AFireProjHostile>(), npc.damage / (Main.expertMode ? 2 : 4), 3,
                        Main.myPlayer);
                }
            }
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            if (npc.ai[1] == 1 || npc.ai[2] >= 470 || Main.npc[(int) npc.ai[3]].ai[1] == 1 ||
                Main.npc[(int) npc.ai[3]].ai[2] >= 500)
            {
            ...
            Texture2D myGlowTex = (npc.type == mod.NPCType<AkumaA>() ? HeadGlow :
                npc.type == mod.NPCType<AkumaAArms>() ? glowTex2 :
                npc.type == mod.NPCType<AkumaABody>() ? glowTex3 :
                npc.type == mod.NPCType<AkumaABody1>() ? glowTex4 : glowTex5);
            var effects = npc.spriteDirection == -1 ? SpriteEffects.None : SpriteEffects.FlipHorizontally;
            spriteBatch.Draw(AkumaTex, Drawpos, npc.frame, npc.GetAlpha(drawColor), npc.rotation, npc.frame.Size() / 2,
                npc.scale, effects, 0);
            BaseDrawing.DrawTexture(spriteBatch, myGlowTex, shader, npc, npc.GetAlpha(Color.White), true,
                npc.frame.Size() / 2);
            return false;
        }
