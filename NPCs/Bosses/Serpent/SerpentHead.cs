    ...
    public class SerpentHead : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Math.Abs(npc.position.X - Main.player[npc.target].position.X) > 6000f ||
                Math.Abs(npc.position.Y - Main.player[npc.target].position.Y) > 6000f || Main.player[npc.target].dead)
            {
                if (StopSnow == 0)
                {
                    RainStop();
            ...
            BaseMod.BaseAI.AIWorm(npc,
                new int[] {mod.NPCType("SerpentHead"), mod.NPCType("SerpentBody"), mod.NPCType("SerpentTail")}, 12, 8f,
                12f, 0.1f, false, false);
        ...
        public void FrostAttack()
        {
            ...
            if (fireAttack == true)
            {
                ...
                if ((attackTimer == 8 || attackTimer == 16 || attackTimer == 24 || attackTimer == 32 ||
                     ...
                     attackTimer == 72 || attackTimer == 79) && !npc.HasBuff(103))
                {
                    for (int i = 0; i < 5; ++i)
                    {
                        int num429 = 1;
                        ...
                        Projectile.NewProjectile(PlayerDistance.X, PlayerDistance.Y, npc.velocity.X * 2f,
                            npc.velocity.Y * 2f, mod.ProjectileType("SnowBreath"), npc.damage, 0, Main.myPlayer);
                    }
                }
