...
namespace AAMod.NPCs.Bosses.Toad
{
    public class TruffleToadTongue : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1 && projectile.ai[0] > 0f &&
                (!Main.player[(int) projectile.ai[1]].active || Main.player[(int) projectile.ai[1]].dead))
            {
                projectile.ai[0] *= -1f;
                projectile.netUpdate = true;
            }
            ...
            projectile.rotation =
                (Main.npc[(int) Math.Abs(projectile.ai[0]) - 1].Center - Main.player[(int) projectile.ai[1]].Center +
                 value43).ToRotation() + 1.57079637f;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            Main.spriteBatch.Draw(texture2D24, projectile.Center - Main.screenPosition, new Rectangle?(rectangle10),
                projectile.GetAlpha(color25), projectile.rotation, rectangle10.Size() / 2f, projectile.scale,
            ...
            if (num240 > 0f)
            {
                ...
                while (num241 + 1f < num240)
                {
                    ...
                    Main.spriteBatch.Draw(texture2D25, vector32 - Main.screenPosition, new Rectangle?(rectangle11),
                        projectile.GetAlpha(color47), projectile.rotation, rectangle11.Bottom(), projectile.scale,
                        SpriteEffects.None, 0f);
            ...
            Main.spriteBatch.Draw(texture2D26, vector32 - Main.screenPosition, new Rectangle?(value36), color48,
                projectile.rotation, new Vector2(value36.Width / 2f, value36.Height), projectile.scale,
                SpriteEffects.None, 0f);
            return false;
        }
    }
