        ...
        public override void Kill(int timeLeft)
        {
            ...
            if (projectile.owner == Main.myPlayer)
            {
                for (i = 0; i < 10; i++)
                {
                    ...
                    Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y,
                        ...
                        mod.ProjectileType("YamataRain"), projectile.damage, projectile.knockBack, projectile.owner, 0f,
                        0f);
                    Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y,
                        (float) (-Math.Sin(offsetAngle) * 6f), (float) (-Math.Cos(offsetAngle) * 6f),
                        mod.ProjectileType("YamataRain"), projectile.damage, projectile.knockBack, projectile.owner, 0f,
            ...
            for (int dust = 0; dust <= 5; dust++)
            {
                Dust.NewDust(projectile.position + projectile.velocity, projectile.width, projectile.height,
                    mod.DustType<Dusts.CthulhuAuraDust>(), projectile.oldVelocity.X * 0.5f,
