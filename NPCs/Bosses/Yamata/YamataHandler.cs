...
namespace AAMod
{
    public abstract class YamataBoss : ParentNPC
    {
        ...
        public string name
        {
            get { return npc.TypeName; }
            set { }
        }

        public string displayName
        {
            get { return DisplayName.GetDefault(); }
        ...
        public override bool? DrawHealthBar(byte hbPosition, ref float scale, ref Vector2 position)
        {
            ...
            else if (realLifeHealthBar)
            {
                ...
                else
                {
                    ...
                    Main.instance.DrawHealthBar(position.X, position.Y, Main.npc[npc.realLife].life,
                        Main.npc[npc.realLife].lifeMax, alpha, scale);
                    npc.position -= npc.visualOffset;
                    result = new bool?(false);
                }
            }
        ...
        {
            float result;
            if (!invasionSpawn && (Main.invasionType > 0 || Main.pumpkinMoon || Main.snowMoon || Main.bloodMoon ||
                                   Main.eclipse || DD2Event.Ongoing ||
                                   BaseExtensions.InZone(spawnInfo.player, "TowerAny", null)))
            {
                result = 0f;
            }
            else if (!specialBiomeSpawn && (BaseExtensions.InZone(spawnInfo.player, "TowerAny", null) ||
                                            BaseExtensions.InZone(spawnInfo.player, "Dungeon", null) ||
                                            BaseExtensions.InZone(spawnInfo.player, "Meteor", null) ||
                                            spawnInfo.lihzahrd))
            {
                result = 0f;
            }
            ...
            {
                result = (G_CanSpawn(spawnInfo.spawnTileX, spawnInfo.spawnTileY, npc.type, spawnInfo.player, spawnInfo)
                    ? 1f
                    : 0f);
            }
