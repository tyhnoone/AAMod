...
namespace AAMod.NPCs.Bosses.Yamata
{
    ...
    public class Yamata : YamataBoss
    {
        ...
        public override void NPCLoot()
        {
            ...
            if (!Tag)
            {
                if (!Main.expertMode)
                {
                    ...
                    string[] lootTable =
                    {
                        "Flairdra", "Crescent", "Hydraslayer", "AbyssArrow", "HydraStabber", "MidnightWrath",
                        "YamataTerratool"
                    };
                    ...
                    Main.NewText(
                        "HAH! I went easy on ya! Come back when you’re actually good and we can have a real fight!",
                        new Color(45, 46, 70));
                ...
                if (Main.expertMode)
                {
                    int npcID = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("YamataTransition"), 0,
                        0, 0, 0, 0, npc.target);
        ...
        public override void AI()
        {
            ...
            if (!HeadsSpawned)
            {
                ...
                if (Main.netMode != 1)
                {
                    if (isAwakened)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100, mod.NPCType("YamataAHead"),
                            0, npc.whoAmI);
                        Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                        TrueHead = Main.npc[latestNPC];
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataAHeadF1"), 0, npc.whoAmI);
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                        ...
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataAHeadF2"), 0, npc.whoAmI);
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                    ...
                    {
                        int latestNPC = npc.whoAmI;
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100, mod.NPCType("YamataHead"),
                            0, npc.whoAmI);
                        Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                        TrueHead = Main.npc[latestNPC];
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataHeadF1"), 0, npc.whoAmI);
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                        Head2 = Main.npc[latestNPC];
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataHeadF1"), 0, npc.whoAmI);
                        ...
                        Head4 = Main.npc[latestNPC];
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataHeadF2"), 0, npc.whoAmI);
                        Main.npc[(int) latestNPC].ai[0] = npc.whoAmI;
                        Head5 = Main.npc[latestNPC];
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y - 100,
                            mod.NPCType("YamataHeadF2"), 0, npc.whoAmI);
            ...
            if (Main.dayTime)
            {
                Main.NewText(
                    isAwakened
                        ? "THE SUN DOESN'T SHINE IN THE DEPTHS!!! NYEHEHEHEHEHEHEHEH!!!"
                        : "HISSSSSSSSSSSSSSS!!! THE SUNNNNNNNNNN! I'M OUT!",
                    isAwakened ? new Color(146, 30, 68) : new Color(45, 46, 70));
                if (isAwakened)
                {
                    Main.dayTime = false;
                    Main.time = 0;
            ...
            if (foundTarget || (NPC.AnyNPCs(mod.NPCType<YamataHead>()) || NPC.AnyNPCs(mod.NPCType<YamataAHead>())))
            {
                ...
                if (!NoFly4U && NoFlyCountDown <= 0 && !AAWorld.downedYamata)
                {
                    NoFlyCountDown = 0;
                    NoFly4U = true;
                    playerTarget.AddBuff(
                        isAwakened ? mod.BuffType<Buffs.YamataAGravity>() : mod.BuffType<Buffs.YamataGravity>(), 10,
                        true);
                    if (npc.type == mod.NPCType<Yamata>())
                        BaseUtility.Chat(
                            "Oh and don't even think about flying! My ego is so massive it has a gravitational pull all of it's own! NYEHEHEHEHEHEHEHEHEHEHEHEH!!!",
                            new Color(45, 46, 70));
                }

                float dist = npc.Distance(playerTarget.Center);
                MoveSpeed = dist > 300 ? 6f : 3f;
                if (dist > 800 || !Collision.CanHit(npc.position, npc.width, npc.height,
                        ...
                        Main.player[npc.target].height))
                {
                    if (Main.netMode != 1 && SayTheLineYamata == 300)
                    {
                        ...
                        {
                            BaseUtility.Chat(
                                isAwakened ? "THERE IS NO ESCAPE FROM THE ABYSS!" : "Running away?! I DON'T THINK SO!",
                                isAwakened ? new Color(146, 30, 68) : new Color(45, 46, 70));
                            FirstLine = true;
                        }
                    }
            ...
            bottomVisualOffset =
                new Vector2(Math.Min(3f, Math.Abs(npc.velocity.X)), 0f) * (npc.velocity.X < 0 ? 1 : -1);
            UpdateLimbs();
        }
        ...
        public void AIMovementRunAway()
        {
            ...
            {
                BaseUtility.Chat("NYEHEHEHEHEHEHEH..! And don’t come back!",
                    isAwakened ? new Color(146, 30, 68) : new Color(45, 46, 70));
                loludide = true;
            }
        ...
        public void YamataBody(NPC npc, ref float[] ai, bool ignoreWet = true, float moveInterval = 0.2f,
            float maxSpeedX = 2f, float maxSpeedY = 1.5f, float hoverInterval = 0.04f, float hoverMaxSpeed = 1.5f,
            int hoverHeight = 3)
        {
            ...
            {
                ai[2] += 1f;
                if (Main.player[npc.target].position.X + (float) (Main.player[npc.target].width / 2) >
            ...
            for (int tY = tileY; tY < tileY + hoverHeight; tY++)
            {
                ...
                if ((Main.tile[tileX, tY].nactive() && Main.tileSolid[(int) Main.tile[tileX, tY].type]) ||
                    Main.tile[tileX, tY].liquid > 0)
                {
            ...
            if (!tileBelowEmpty && npc.target > -1 && Main.player[npc.target].active && !Main.player[npc.target].dead &&
                Math.Abs(Main.player[npc.target].Center.X - npc.Center.X) < 50) //force a hover
            {
        ...
        public void DrawHead(SpriteBatch spriteBatch, string headTexture, string glowMaskTexture, NPC head,
            Color drawColor, bool DrawUnder)
        {
            ...
            {
                string neckTex =
                    (isAwakened ? "NPCs/Bosses/Yamata/Awakened/YamataANeck" : "NPCs/Bosses/Yamata/YamataNeck");
                ...
                BaseDrawing.DrawChain(spriteBatch, new Texture2D[] {neckTex2D, neckTex2D, neckTex2D}, 0, neckOrigin,
                    connector, neckTex2D.Height - 10f, lightColor, 1f, DrawUnder, null);
                BaseDrawing.DrawTexture(spriteBatch, mod.GetTexture(headTexture), 0,
                    head.position + new Vector2(0f, head.gfxOffY) + topVisualOffset, head.width, head.height,
                    head.scale, head.rotation, head.spriteDirection, Main.npcFrameCount[head.type], head.frame,
                    lightColor, false);
                BaseDrawing.DrawTexture(spriteBatch, mod.GetTexture(glowMaskTexture), 0,
                    head.position + new Vector2(0f, head.gfxOffY) + topVisualOffset, head.width, head.height,
                    head.scale, head.rotation, head.spriteDirection, Main.npcFrameCount[head.type], head.frame,
                    GlowColor, false);
            }
        ...
        public override void PostDraw(SpriteBatch sb, Color dColor)
        {
            ...
            BaseDrawing.DrawTexture(sb, mod.GetTexture(tailTex), 0,
                npc.position + new Vector2(0f, npc.gfxOffY) + bottomVisualOffset +
                (isAwakened ? new Vector2(0, -32) : new Vector2(0, 0)), npc.width, npc.height, npc.scale, npc.rotation,
                npc.spriteDirection, Main.npcFrameCount[npc.type], frameBottom, lightColor, false);
            if (legs != null && legs.Length == 4)
            {
            ...
            BaseDrawing.DrawTexture(sb, Main.npcTexture[npc.type], 0,
                npc.position + new Vector2(0f, npc.gfxOffY) + topVisualOffset, npc.width, npc.height, npc.scale,
                npc.rotation, npc.spriteDirection, Main.npcFrameCount[npc.type], npc.frame, lightColor, false);
            if (isAwakened)
            {
                BaseDrawing.DrawTexture(sb, mod.GetTexture(glowTex), 0,
                    npc.position + new Vector2(0f, npc.gfxOffY) + topVisualOffset, npc.width, npc.height, npc.scale,
                    npc.rotation, npc.spriteDirection, Main.npcFrameCount[npc.type], npc.frame, AAColor.YamataA, false);
            }
            ...
            {
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF1", "Glowmasks/YamataAHeadF_Glow", Head2, dColor,
                    false);
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF1", "Glowmasks/YamataAHeadF_Glow", Head3, dColor,
                    false);
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF1", "Glowmasks/YamataAHeadF_Glow", Head4, dColor,
                    false);
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF2", "Glowmasks/YamataAHeadF_Glow", Head5, dColor,
                    false);
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF2", "Glowmasks/YamataAHeadF_Glow", Head6, dColor,
                    false);
                DrawHead(sb, "NPCs/Bosses/Yamata/Awakened/YamataAHeadF2", "Glowmasks/YamataAHeadF_Glow", Head7, dColor,
                    false);
            ...
            if (isAwakened)
            {
                BaseDrawing.DrawAfterimage(sb, mod.GetTexture(glowTex), 0, npc, 0.8f, 1f, 4, false, 0f, 0f,
                    AAColor.YamataA);
            }
        }
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (!AAWorld.downedYamata)
            {
                ...
                {
                    Main.NewText(
                        "Resistance isn't gonna save you here! Now stop being a little brat and let me destroy you!",
                        new Color(45, 46, 70));
            ...
            if (AAWorld.downedYamata)
            {
                ...
                {
                    Main.NewText(
                        "I don't understand why you keep fighting me! I'm superior to you in every single way!",
                        new Color(45, 46, 70));
                    threeQuarterHealth = true;
                }
    ...
    public class LimbInfo
    {
        ...
        public Vector2 Center
        {
            get
            {
                return new Vector2(position.X + ((float) Hitbox.Width * 0.5f),
                    position.Y + ((float) Hitbox.Height * 0.5f));
            }
            set
            {
                position = new Vector2(value.X - ((float) Hitbox.Width * 0.5f),
                    value.Y - ((float) Hitbox.Height * 0.5f));
            }
        }
    ...
    public class LegInfo : LimbInfo
    {
        ...
        {
            Vector2 movementSpot = GetBodyConnector(npc) +
                                   new Vector2(
                                       (limbType == 3 ? (-35f - Hitbox.Width) :
                                           limbType == 2 ? 35f :
                                           limbType == 1 ? (-15f - Hitbox.Width) : 15f),
                                       (limbType == 1 || limbType == 0 ? 40f : 50f));
            ...
            else if (Vector2.Distance(movementSpot, position) > (40 + (int) npc.velocity.Length()))
            {
                Vector2 velAddon = (movementSpot - position);
                velAddon.Normalize();
                velAddon *= (2f + (velLength * 0.25f));
                velocity += velAddon;
        ...
        public void MoveLegWalking(NPC npc, bool leftLeg, Vector2 standOnPoint)
        {
            ...
            {
                Vector2 velAddon = (pointToStandOn - position);
                velAddon.Normalize();
                velAddon *= (1.6f + (npc.velocity.Length() * 0.5f));
                velocity += velAddon;
                ...
                if (Vector2.Distance(pointToStandOn, position) <= 15)
                {
                    position = pointToStandOn;
                    velocity = default(Vector2);
                }

                position += velocity;
                if ((position == pointToStandOn ||
                     Vector2.Distance(standOnPoint, position + new Vector2((float) Hitbox.Width * 0.5f, 0f)) >
                     distanceToMove || Math.Abs(position.X - standOnPoint.X) > distanceToMoveX))
                {
            ...
            if (pointToStandOn == default(Vector2))
            {
                if (Vector2.Distance(standOnPoint, position + new Vector2((float) Hitbox.Width * 0.5f, 0f)) >
                    distanceToMove || Math.Abs(position.X - standOnPoint.X) > distanceToMoveX)
                {
                    movementRatio = 0f;
        ...
        public void UpdateLeg(NPC npc)
        {
            ...
            if (Vector2.Distance(Center, npc.Center) > 499 || Yamata.TeleportMeBitch)
                position = npc.Center; //prevent issues when the legs are WAY off.
        ...
        public Vector2 GetStandOnPoint(NPC npc)
        {
            ...
            float rightLegScalar =
                1f + (npc.velocity.X > 2f ? (scalar * 0.2f) : 0f); //fixes an offset problem when the npc walks right
            float standOnX = npc.Center.X + yamata.topVisualOffset.X + (limbType == 3
                                 ?
                                 (-outerLegDefault - Hitbox.Width)
                                 : limbType == 2
                                     ? (outerLegDefault + Hitbox.Width)
                                     : limbType == 1
            ...
            if (!flying)
            {
                ...
                if (Main.tile[(int) (standOnX / 16f), tileY] == null ||
                    !Main.tile[(int) (standOnX / 16f), tileY].nactive() ||
                    !Main.tileSolid[Main.tile[(int) (standOnX / 16f), tileY].type]) tilePosY += 16f;
        ...
        public void DrawLeg(SpriteBatch sb, NPC npc, Color dColor)
        {
            ...
            if (!leftLeg)
            {
                BaseDrawing.DrawChain(sb, new Texture2D[] {null, textures[3], null}, 0,
                    drawPos + new Vector2(Hitbox.Width * 0.5f, 6f), legJoint, 0f, null, 1f, false, null);
                BaseDrawing.DrawChain(sb, new Texture2D[] {textures[2], textures[3], textures[2]}, 0, legJoint,
                    GetBodyConnector(npc), 0f, null, 1f, false, null);
            }
            else
            {
                BaseDrawing.DrawChain(sb, new Texture2D[] {null, textures[1], null}, 0,
                    drawPos + new Vector2(Hitbox.Width * 0.5f, 6f), legJoint, 0f, null, 1f, false, null);
                BaseDrawing.DrawChain(sb, new Texture2D[] {textures[0], textures[1], textures[0]}, 0, legJoint,
                    GetBodyConnector(npc), 0f, null, 1f, false, null);
            }

            BaseDrawing.DrawTexture(sb, textures[4], 0, drawPos, Hitbox.Width, Hitbox.Height, npc.scale, rotation,
                limbType == 1 || limbType == 3 ? 1 : -1, 1, Hitbox, lightColor, false, legOrigin);
        }
    }
}