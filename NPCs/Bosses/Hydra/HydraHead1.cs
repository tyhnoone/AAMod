...
namespace AAMod.NPCs.Bosses.Hydra
{
    ...
    public class HydraHead1 : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (Main.netMode != 1)
            {
                ...
                if (targetPlayer != null)
                {
                    ...
                    if (!middleHead && npc.ai[1] == aiTimerFire)
                    {
                        ...
                        for (int i = 0; i < 7; ++i)
                        {
                            int projID = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, dir.X, dir.Y,
                                mod.ProjectileType("AcidProj"), (int) (damage * .8f), 0f, Main.myPlayer);
                            Main.projectile[projID].netUpdate = true;
                        }
                    }
                    else if (middleHead && npc.ai[1] >= 100 && npc.ai[1] % 10 == 0)
                    {
                        dir *= 6f;
                        dir.X += Main.rand.Next(-1, 1) * 0.5f;
                        dir.Y += Main.rand.Next(-1, 1) * 0.5f;
                        int projID = Projectile.NewProjectile(npc.Center.X, npc.Center.Y, dir.X, dir.Y,
                            mod.ProjectileType("HydraBreath"), (int) (damage * .8f), 0f, Main.myPlayer);
                        Main.projectile[projID].netUpdate = true;
                    }
                }
            ...
            npc.rotation = 1.57f;
            Vector2 nextTarget = Body.npc.Center +
                                 new Vector2(middleHead ? 0 : leftHead ? -distFromBodyX : distFromBodyX,
                                     -distFromBodyY) + new Vector2(npc.ai[2], npc.ai[3]);
