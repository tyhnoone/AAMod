...
namespace AAMod.NPCs.Bosses.GripsShen
{
    public abstract class BaseShenGrips : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            else if (npc.ai[0] == 4) //dive back down
            {
                moveSpeed = 30f;
                Vector2 targetCenter = new Vector2(npc.ai[1], npc.ai[2]);
                Vector2 point = targetCenter - offsetBasePoint + new Vector2(0f, 250f);
            ...
            else if (npc.ai[0] == 5) //Fire Projectile (Shen Grips)
            {
                moveSpeed = 15f;
                Vector2 point = targetPlayer.Center + offsetBasePoint + new Vector2(0f, -250f);
                MoveToPoint(point);
                ...
                if (Main.netMode != 1 && (Vector2.Distance(npc.Center, point) < 10f || forceChange))
                {
                    ...
                    if (internalAI[0] == 40)
                    {
                        BaseAI.FireProjectile(targetPlayer.Center, npc.Center,
                            BlazeGrip ? mod.ProjectileType<BlazeBomb>() : mod.ProjectileType<AbyssalBomb>(),
                            (int) (npc.damage * (Main.expertMode ? .25f : .5f)), 2, 9f, -1, Main.myPlayer);
                    }
        ...
        private void DespawnHandler()
        {
            ...
            ) // If the player is dead and not active, the npc flies off-screen and despawns
            {
                npc.velocity.X = 0;
                npc.velocity.Y -= 1;
            }
