    ...
    public class Exterminator : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            for (int num1011 = 0; num1011 < 2; num1011++)
            {
                ...
                {
                    int num1012 = Dust.NewDust(npc.Center - new Vector2((float) num1010), num1010 * 2, num1010 * 2,
                        mod.DustType<Dusts.VoidDust>(), npc.velocity.X * 0.5f, npc.velocity.Y * 0.5f, 90,
                        default(Color), 1.5f);
            ...
            if (npc.ai[0] == 0f)
            {
                ...
                float num1014 = 8f;
                flag64 = (flag64 && vector126.ToRotation() > 3.14159274f / num1014 &&
                          vector126.ToRotation() < 3.14159274f - 3.14159274f / num1014);
            ...
            else if (npc.ai[0] == 1f)
            {
                ...
                if (npc.ai[1] >= num1001)
                {
                    ...
                    Vector2 velocity = new Vector2(npc.ai[2], npc.ai[3]) +
                                       new Vector2((float) Main.rand.Next(-num1002, num1002 + 1),
                                           (float) Main.rand.Next(-num1002, num1002 + 1)) * 0.04f;
                    velocity.Normalize();
            ...
            else if (npc.ai[0] == 2f)
            {
                ...
                npc.ai[1] += 1f;
                bool flag65 = Vector2.Distance(npc.Center, Main.player[npc.target].Center) > num1004 &&
            ...
            if (npc.ai[0] == 3f)
            {
                ...
                {
                    int num1018 = Dust.NewDust(npc.position, npc.width, npc.height, mod.DustType<Dusts.VoidDust>(), 0f,
                        0f, 100, default(Color), 1.5f);
                    Main.dust[num1018].velocity *= 1.4f;
                    Main.dust[num1018].position =
                        ((float) Main.rand.NextDouble() * 6.28318548f).ToRotationVector2() *
                        ((float) Main.rand.NextDouble() * 96f) + npc.Center;
                }

                for (int num1019 = 0; num1019 < 40; num1019++)
                {
                    int num1020 = Dust.NewDust(npc.position, npc.width, npc.height, 226, 0f, 0f, 100, default(Color),
                        0.5f);
                    Main.dust[num1020].noGravity = true;
                    Main.dust[num1020].velocity *= 2f;
                    Main.dust[num1020].position =
                        ((float) Main.rand.NextDouble() * 6.28318548f).ToRotationVector2() *
                        ((float) Main.rand.NextDouble() * 96f) + npc.Center;
                    Main.dust[num1020].velocity = Main.dust[num1020].velocity / 2f +
                                                  Vector2.Normalize(Main.dust[num1020].position - npc.Center);
                    if (Main.rand.Next(2) == 0)
                    {
                        num1020 = Dust.NewDust(npc.position, npc.width, npc.height, 226, 0f, 0f, 100, default(Color),
                            0.9f);
                        Main.dust[num1020].noGravity = true;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            Vector2 vector10 = new Vector2((float) (Main.npcTexture[npc.type].Width / 2),
                (float) (Main.npcTexture[npc.type].Height / Main.npcFrameCount[npc.type] / 2));
            Color color9 = Lighting.GetColor((int) ((double) npc.position.X + (double) npc.width * 0.5) / 16,
            ...
            if ((npc.ai[0] == 2f || npc.ai[0] == 4f))
            {
                ...
                for (int num213 = 6; num213 >= 0; num213--)
                {
                    ...
                    Main.spriteBatch.Draw(texture2D32,
                        new Vector2(
                            npc.oldPos[num213].X - Main.screenPosition.X + (float) (npc.width / 2) -
                            (float) Main.npcTexture[npc.type].Width * npc.scale / 2f + vector10.X * npc.scale,
                            npc.oldPos[num213].Y - Main.screenPosition.Y + (float) npc.height -
                            (float) Main.npcTexture[npc.type].Height * npc.scale /
                            (float) Main.npcFrameCount[npc.type] + 4f + vector10.Y * npc.scale + num69),
                        new Rectangle?(value36), color47, npc.oldRot[num213] + num211, origin16,
                        MathHelper.Lerp(0.1f, 1.2f, (10f - (float) num213) / 10f), SpriteEffects.None, 0f);
                }
            }

            Main.spriteBatch.Draw(mod.GetTexture("Glowmasks/Exterminator_Glow"),
                npc.Bottom - Main.screenPosition + new Vector2(
                    (float) (-(float) Main.npcTexture[npc.type].Width) * npc.scale / 2f + vector10.X * npc.scale,
                    (float) (-(float) Main.npcTexture[npc.type].Height) * npc.scale /
                    (float) Main.npcFrameCount[npc.type] + 4f + vector10.Y * npc.scale + num69 + npc.gfxOffY),
                new Microsoft.Xna.Framework.Rectangle?(npc.frame),
                new Microsoft.Xna.Framework.Color(255 - npc.alpha, 255 - npc.alpha, 255 - npc.alpha, 255 - npc.alpha),
                npc.rotation, vector10, npc.scale, SpriteEffects.None, 0f);
            ...
            for (int num247 = 0; num247 < 4; num247++)
            {
                Main.spriteBatch.Draw(mod.GetTexture("Glowmasks/Exterminator_Glow"),
                    ...
                    new Vector2(
                        ...
                        (float) (-(float) Main.npcTexture[npc.type].Height) * npc.scale /
                        (float) Main.npcFrameCount[npc.type] + 4f + vector10.Y * npc.scale + num69 + npc.gfxOffY) +
                    npc.velocity.RotatedBy((double) ((float) num247 * 1.57079637f), default(Vector2)) * scaleFactor14,
                    new Microsoft.Xna.Framework.Rectangle?(npc.frame), new Microsoft.Xna.Framework.Color(64, 64, 64, 0),
                    npc.rotation, vector10, npc.scale, SpriteEffects.None, 0f);
            }
