        ...
        public override void AI()
        {
            ...
            if (npc.velocity.Y != 0f && npc.ai[2] == 1f)
            {
                ...
                if (Main.rand.Next(3) == 0)
                {
                    ...
                    Vector2 velocity = new Vector2((float) (npc.direction * -6), 12f) * 0.2f +
                                       Utils.RandomVector2(Main.rand, -1f, 1f) * 0.1f;
                    Dust dust6 =
                        Main.dust[
                            Dust.NewDust(position, 8, 8, 229, velocity.X, velocity.Y, 100, Color.Transparent,
                                1f + Main.rand.NextFloat() * 0.5f)];
                ...
                {
                    if (num84 != npc.whoAmI && Main.npc[num84].active && Main.npc[num84].type == npc.type &&
                        Math.Abs(npc.position.X - Main.npc[num84].position.X) +
                        Math.Abs(npc.position.Y - Main.npc[num84].position.Y) < (float) npc.width)
                    {
            ...
            else if (Main.player[npc.target].Center.Y + 100f < npc.position.Y && Collision.CanHit(npc.position,
                         npc.width, npc.height, Main.player[npc.target].position, Main.player[npc.target].width,
            ...
            {
                npc.localAI[2] += 1f;
                if (npc.localAI[2] >= (float) (360 + Main.rand.Next(360)) &&
                    npc.Distance(Main.player[npc.target].Center) < 400f &&
                    Math.Abs(npc.DirectionTo(Main.player[npc.target].Center).Y) < 0.5f &&
                    Collision.CanHitLine(npc.Center, 0, 0, Main.player[npc.target].Center, 0, 0))
                {
                    ...
                    for (int num86 = 0; num86 < 4; num86++)
                    {
                        ...
                        Projectile.NewProjectile(vector13.X, vector13.Y, vector15.X - 5, vector15.Y - 5,
                            mod.ProjectileType<TerrariumArrow>(), num85, 1f, Main.myPlayer, 0f, 0f);
                    }
                }
            }
            ...
            if (npc.velocity.Y >= 0f)
            {
                ...
                int num172 = (int) ((position2.X + (float) (npc.width / 2) + (float) ((npc.width / 2 + 1) * num171)) /
                                    16f);
                ...
                if ((float) (num172 * 16) < position2.X + (float) npc.width &&
                    (float) (num172 * 16 + 16) > position2.X &&
                    ((Main.tile[num172, num173].nactive() && !Main.tile[num172, num173].topSlope() &&
                      !Main.tile[num172, num173 - 1].topSlope() &&
                      Main.tileSolid[(int) Main.tile[num172, num173].type] &&
                      !Main.tileSolidTop[(int) Main.tile[num172, num173].type]) ||
                     (Main.tile[num172, num173 - 1].halfBrick() && Main.tile[num172, num173 - 1].nactive())) &&
                    (!Main.tile[num172, num173 - 1].nactive() ||
                     !Main.tileSolid[(int) Main.tile[num172, num173 - 1].type] ||
                     Main.tileSolidTop[(int) Main.tile[num172, num173 - 1].type] ||
                     (Main.tile[num172, num173 - 1].halfBrick() &&
                      (!Main.tile[num172, num173 - 4].nactive() ||
                       !Main.tileSolid[(int) Main.tile[num172, num173 - 4].type] ||
                       Main.tileSolidTop[(int) Main.tile[num172, num173 - 4].type]))) &&
                    (!Main.tile[num172, num173 - 2].nactive() ||
                     !Main.tileSolid[(int) Main.tile[num172, num173 - 2].type] ||
                     Main.tileSolidTop[(int) Main.tile[num172, num173 - 2].type]) &&
                    (!Main.tile[num172, num173 - 3].nactive() ||
                     !Main.tileSolid[(int) Main.tile[num172, num173 - 3].type] ||
                     Main.tileSolidTop[(int) Main.tile[num172, num173 - 3].type]) &&
                    (!Main.tile[num172 - num171, num173 - 3].nactive() ||
                     !Main.tileSolid[(int) Main.tile[num172 - num171, num173 - 3].type]))
                {
            ...
            if (flag23)
            {
                int num177 = (int) ((npc.position.X + (float) (npc.width / 2) +
                                     (float) ((npc.width / 2 + 16) * npc.direction)) / 16f);
                ...
                if (Main.tile[num177, num178 - 1].nactive() &&
                    (Main.tile[num177, num178 - 1].type == 10 || Main.tile[num177, num178 - 1].type == 388) && flag6)
                {
                    npc.ai[2] += 1f;
                    npc.ai[3] = 0f;
                    if (npc.ai[2] >= 60f)
                ...
                else
                {
                    ...
                    {
                        if (npc.height >= 32 && Main.tile[num177, num178 - 2].nactive() &&
                            Main.tileSolid[(int) Main.tile[num177, num178 - 2].type])
                        {
                            if (Main.tile[num177, num178 - 3].nactive() &&
                                Main.tileSolid[(int) Main.tile[num177, num178 - 3].type])
                            {
                                npc.velocity.Y = -8f;
                                npc.netUpdate = true;
                            }
                        ...
                        else if (Main.tile[num177, num178 - 1].nactive() &&
                                 Main.tileSolid[(int) Main.tile[num177, num178 - 1].type])
                        {
                            npc.velocity.Y = -6f;
                            npc.netUpdate = true;
                        }
                        else if (npc.position.Y + (float) npc.height - (float) (num178 * 16) > 20f &&
                                 Main.tile[num177, num178].nactive() && !Main.tile[num177, num178].topSlope() &&
                                 Main.tileSolid[(int) Main.tile[num177, num178].type])
                        {
                            npc.velocity.Y = -5f;
                            npc.netUpdate = true;
                        }
                        else if (npc.directionY < 0 &&
                                 (!Main.tile[num177, num178 + 1].nactive() ||
                                  !Main.tileSolid[(int) Main.tile[num177, num178 + 1].type]) &&
                                 (!Main.tile[num177 + npc.direction, num178 + 1].nactive() ||
                                  !Main.tileSolid[(int) Main.tile[num177 + npc.direction, num178 + 1].type]))
                        {
                            npc.velocity.Y = -8f;
        ...
        public override void NPCLoot()
        {
            if (Main.rand.Next(100) == 0)
            {
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height,
                    mod.ItemType<Items.Materials.TerraCrystal>());
            }
        }
    }
}