...
namespace AAMod.NPCs.Enemies.Terrarium.PostEquinox
{
    public class TerraSerpent : ModNPC
    {
        ...
        {
            get { return "AAMod/NPCs/Enemies/Terrarium/PostEquinox/TerraSerpent"; }
        }
        ...
        public override bool PreAI()
        {
            ...
            if (npc.alpha != 0)
            {
                ...
                {
                    int num935 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height, 107,
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    {
                        latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType<TerraSerpentBody>(),
                            npc.whoAmI, 0, latestNPC);
                        Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int) latestNPC].ai[3] = npc.whoAmI;
                    }

                    latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType<TerraSerpentTail>(),
                        npc.whoAmI, 0, latestNPC);
                    Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int) latestNPC].ai[3] = npc.whoAmI;
            ...
            for (int i = minTilePosX; i < maxTilePosX; ++i)
            {
                for (int j = minTilePosY; j < maxTilePosY; ++j)
                {
                    if (Main.tile[i, j] != null &&
                        (Main.tile[i, j].nactive() && (Main.tileSolid[(int) Main.tile[i, j].type] ||
                                                       Main.tileSolidTop[(int) Main.tile[i, j].type] &&
                                                       (int) Main.tile[i, j].frameY == 0) ||
                         (int) Main.tile[i, j].liquid > 64))
                    {
                        Vector2 vector2;
        ...
        public override bool PreDraw(SpriteBatch spriteBatch, Color drawColor)
        {
            ...
            spriteBatch.Draw(BodyTex, Drawpos, npc.frame, npc.GetAlpha(drawColor), npc.rotation, npc.frame.Size() / 2,
                npc.scale, SpriteEffects.None, 0);
            BaseDrawing.DrawTexture(spriteBatch, glowTex1, shader, npc, npc.GetAlpha(Color.White), true,
                npc.frame.Size() / 2);
            BaseDrawing.DrawTexture(spriteBatch, glowTex2, 0, npc, npc.GetAlpha(Color.White), true,
                npc.frame.Size() / 2);
    ...
    public class TerraSerpentBody : TerraSerpent
    {
        public override string Texture
        {
            get { return "AAMod/NPCs/Enemies/Terrarium/PostEquinox/TerraSerpentBody"; }
        }
    ...
    public class TerraSerpentTail : TerraSerpent
    {
        public override string Texture
        {
            get { return "AAMod/NPCs/Enemies/Terrarium/PostEquinox/TerraSerpentTail"; }
        }
