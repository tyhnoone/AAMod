...
namespace AAMod.NPCs.Enemies.Inferno
{
    public class Dragron : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            npc.TargetClosest(true);
            if (npc.direction == -1 && npc.velocity.X > -4f && npc.position.X >
                Main.player[npc.target].position.X + (float) Main.player[npc.target].width)
            {
                ...
                if (npc.velocity.X < -4f)
                {
                    npc.velocity.X = -4f;
                     ...
                     npc.position.X + (float) npc.width < Main.player[npc.target].position.X)
            {
                npc.velocity.X = npc.velocity.X + 0.08f;
                if (npc.velocity.X < -4f)
                {
            ...
            if (npc.directionY == -1 && (double) npc.velocity.Y > -2.5 && npc.position.Y >
                Main.player[npc.target].position.Y + (float) Main.player[npc.target].height)
            {
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            if (npc.life > 0)
            {
                ...
                {
                    int num590 = Dust.NewDust(npc.position, npc.width, npc.height,
                        mod.DustType<Dusts.BroodmotherDust>(), 0f, 0f, 0, default(Color), 1.5f);
            ...
            {
                int num592 = Dust.NewDust(npc.position, npc.width, npc.height, mod.DustType<Dusts.BroodmotherDust>(),
                    0f, 0f, 0, default(Color), 1.5f);
            ...
            {
                int num594 = Gore.NewGore(new Vector2(npc.position.X, npc.position.Y + (float) (npc.height / 2) - 10f),
                    new Vector2((float) hitDirection, 0f), 99, npc.scale);
        ...
        public override void NPCLoot()
        {
            ...
            {
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height, 3532, 1, false, 0,
                    false, false);
            }
        }
    }
