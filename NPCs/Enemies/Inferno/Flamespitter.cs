            ...
            else if (npc.ai[0] >= teleportInterval && Main.netMode != 1)
            {
                ...
                if (teleport == true)
                {
                    ...
                    while (!hasTeleportPoint && teleportCheckCount < 100)
                    {
                        ...
                        {
                            if ((tpY < playerTileY - 4 || tpY > playerTileY + 4 || tpTileX < playerTileX - 4 ||
                                 tpTileX > playerTileX + 4) &&
                                (tpY < tileY - 1 || tpY > tileY + 1 || tpTileX < tileX - 1 || tpTileX > tileX + 1) &&
                                (!checkGround || Main.tile[tpTileX, tpY].nactive()))
                            {
                                if ((CanTeleportTo != null && CanTeleportTo(tpTileX, tpY)) ||
                                    (!Main.tile[tpTileX, tpY - 1].lava() &&
                                     (!checkGround || Main.tileSolid[(int) Main.tile[tpTileX, tpY].type]) &&
