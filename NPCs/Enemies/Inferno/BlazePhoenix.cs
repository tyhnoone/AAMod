...
namespace AAMod.NPCs.Enemies.Inferno
{
    public class BlazePhoenix : ModNPC
    {
        ...
        public override void AI()
        {
            ...
            if (npc.localAI[0] < num1276)
            {
                ...
                while ((float) num1281 < num1280)
                {
                    ...
                    {
                        int num1282 = Dust.NewDust(npc.position, npc.width, npc.height,
                            mod.DustType<Dusts.DragonflameDust>(), 0f, 0f, 0, default(Color), 1f);
        ...
        public override void NPCLoot()
        {
            if (Main.netMode != 1)
            {
                Item.NewItem((int) npc.position.X, (int) npc.position.Y, npc.width, npc.height,
                    mod.ItemType("DragonFire"), 1 + Main.rand.Next(2));
            }
        }
        ...
        public override bool PreDraw(SpriteBatch spritebatch, Color dColor)
        {
            ...
            BaseDrawing.DrawAfterimage(spritebatch, Main.npcTexture[npc.type], 0, npc, 0.8f, 1f, 4, false, 0f, 0f,
                GetGlowAlpha());
        ...
        public override void HitEffect(int hitDirection, double damage)
        {
            ...
            if (isDead)
            {
                for (int m = 0; m < 30; m++)
                {
                    int dustID = Dust.NewDust(new Vector2(npc.Center.X, npc.Center.Y), npc.width, 1, DustID.Fire,
                        -npc.velocity.X * 0.2f,
                        -npc.velocity.Y * 0.2f, 100, default(Color), 2f);
                    Main.dust[dustID].velocity *= 2f;
                    dustID = Dust.NewDust(new Vector2(npc.Center.X, npc.Center.Y), npc.width, npc.height,
                        mod.DustType<Dusts.BroodmotherDust>(), -npc.velocity.X * 0.2f,
            ...
            for (int m = 0; m < 5; m++)
            {
                Dust.NewDust(npc.position, npc.width, npc.height, DustID.Fire, npc.velocity.X * 0.2f,
                    npc.velocity.Y * 0.2f, 100, Color.White, 1.3f);
            }
        }
    }
}