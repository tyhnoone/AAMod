...
namespace AAMod.NPCs.Bosses.Wyrmling
{
    public class Wyrmling : ModNPC
    {
        ...
        {
            get { return "AAMod/NPCs/Enemies/Inferno/WyrmlingHead"; }
        }
            ...
            if (Main.netMode != 1)
            {
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < WyrmlingLength; ++i)
                    {
                        ...
                        {
                            latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("WyrmlingBody"),
                        ...
                        if (segment == 2)
                        {
                            latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("WyrmlingTail1"),
                                npc.whoAmI, 0, latestNPC);
                            Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                            Main.npc[(int) latestNPC].ai[3] = npc.whoAmI;
                            segment += 1;
                    ...
                    latestNPC = NPC.NewNPC((int) npc.Center.X, (int) npc.Center.Y, mod.NPCType("WyrmlingTail2"),
                        npc.whoAmI, 0, latestNPC);
                    Main.npc[(int) latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int) latestNPC].ai[3] = npc.whoAmI;
    ...
    public class WyrmlingTail1 : Wyrmling
    {
        public override string Texture
        {
            get { return "AAMod/NPCs/Enemies/Inferno/WyrmlingTail1"; }
        }
        ...
        public override bool PreAI()
        {
            ...
            if (Main.npc[(int) npc.ai[1]].alpha < 128)
            {
                if (npc.alpha != 0)
                {
                    ...
                    {
                        int num935 = Dust.NewDust(new Vector2(npc.position.X, npc.position.Y), npc.width, npc.height,
                            mod.DustType("WyrmlingADust"), 0f, 0f, 100, default(Color), 2f);
    ...
    public class WyrmlingBody : Wyrmling
    {
        public override string Texture
        {
            get { return "AAMod/NPCs/Enemies/Inferno/WyrmlingBody"; }
        }
    ...
    public class WyrmlingTail2 : Wyrmling
    {
        public override string Texture
        {
            get { return "AAMod/NPCs/Enemies/Inferno/WyrmlingTail2"; }
        }
