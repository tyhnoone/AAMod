    ...
    public class StoneHydra : ModProjectile
    {
        ...
        public override void AI()
        {
            if (projectile.localAI[0] == 0f)
            {
                ...
                for (int num502 = 0; num502 < num501; num502++)
                {
                    int num503 = Dust.NewDust(new Vector2(projectile.position.X, projectile.position.Y + 16f),
                        projectile.width, projectile.height - 16, 185, 0f, 0f, 0, default(Color), 1f);
            ...
            if (ownerMinionAttackTargetNPC != null && ownerMinionAttackTargetNPC.CanBeChasedBy(this, false))
            {
                ...
                float num511 = Math.Abs(projectile.position.X + (float) (projectile.width / 2) - num509) +
                               Math.Abs(projectile.position.Y + (float) (projectile.height / 2) - num510);
                if (num511 < num508 && Collision.CanHit(projectile.position, projectile.width, projectile.height,
            ...
            if (!flag18)
            {
                for (int num512 = 0; num512 < 200; num512++)
                {
                    if (Main.npc[num512].CanBeChasedBy(this, false))
                    {
                        ...
                        float num515 = Math.Abs(projectile.position.X + (float) (projectile.width / 2) - num513) +
                                       Math.Abs(projectile.position.Y + (float) (projectile.height / 2) - num514);
                        if (num515 < num508 && Collision.CanHit(projectile.position, projectile.width,
                                projectile.height, Main.npc[num512].position, Main.npc[num512].width,
                                Main.npc[num512].height))
                        {
                            num508 = num515;
                            num506 = num513;
                            num507 = num514;
            ...
            if (flag18)
            {
                ...
                if (projectile.ai[0] <= 0f)
                {
                    ...
                    if (Main.myPlayer == projectile.owner)
                    {
                        ...
                        Vector2 vector37 = new Vector2(projectile.position.X + (float) projectile.width * 0.5f,
                        ...
                        Projectile.NewProjectile(vector37.X, vector37.Y, num523 * 1.5f, num524 * 1.5f, num522, num526,
                            projectile.knockBack, Main.myPlayer, 0f, 0f);
                    }
                }
            }
            else if (projectile.ai[0] <= 60f && (projectile.frame == 1 || projectile.frame == 3 ||
                                                 projectile.frame == 5 || projectile.frame == 7 ||
                                                 projectile.frame == 9))
            {
                projectile.frame--;
            }
