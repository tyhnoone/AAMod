...
50% chance to not consume ammo");
            ...
            ref float speedY, ref int type, ref int damage, ref float knockBack)
        {
            ...
            Projectile.NewProjectile(position.X, position.Y, SpeedX, SpeedY, mod.ProjectileType<Moonblow>(), damage,
                knockBack, player.whoAmI, 0.0f, 0.0f);
            for (int i = 0; i <= 6; i++)
            {
                float SpeedNewX = speedX + (float) Main.rand.Next(-45, 46) * 0.05f;
                float SpeedNewY = speedY + (float) Main.rand.Next(-45, 46) * 0.05f;
                Projectile.NewProjectile(position.X, position.Y, SpeedNewX, SpeedNewY, type, damage, knockBack,
                    player.whoAmI, 0.0f, 0.0f);
            }
