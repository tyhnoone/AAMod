                ...
                case 0:
                    for (int i = 0; i < numberProjectiles; i++)
                    {
                        Vector2 perturbedSpeed =
                            new Vector2(speedX * 5, speedY * 5).RotatedBy(MathHelper.Lerp(-rotation, rotation,
                                i / (numberProjectiles - 1))) * .2f;
                        Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y,
                            mod.ProjectileType("FlamingTwilightP"), damage * 4, knockBack, player.whoAmI);
                    }
                    ...
                    for (int i = 0; i < numberProjectiles; i++)
                    {
                        Vector2 perturbedSpeed =
                            new Vector2(speedX * 5, speedY * 5).RotatedBy(MathHelper.Lerp(-rotation, rotation,
                                i / (numberProjectiles - 1))) * .2f;
                        Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y,
                            mod.ProjectileType("FlamingTwilightPD"), damage * 4, knockBack, player.whoAmI);
                    }
                    ...
                    for (int i = 0; i < numberProjectiles; i++)
                    {
                        Vector2 perturbedSpeed =
                            new Vector2(speedX * 5, speedY * 5).RotatedBy(MathHelper.Lerp(-rotation, rotation,
                                i / (numberProjectiles - 1))) * .2f;
                        Projectile.NewProjectile(position.X, position.Y, perturbedSpeed.X, perturbedSpeed.Y,
                            mod.ProjectileType("FlamingTwilightP2"), damage * 4, knockBack, player.whoAmI);
                    }
